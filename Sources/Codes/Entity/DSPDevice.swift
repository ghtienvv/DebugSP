...
public class DSPDevice {
    public static let current: DSPDevice = .init()
    ...
    public func networkUsage() -> DSPNetworkUsage? {
        DSPNetwork.usage()
    }
    ...
    public var localizedGPUMemory: String {
        ...
        formatter.countStyle = .memory
        formatter.allowsNonnumericFormatting = false
        return formatter.string(fromByteCount: Int64(DSPGPU.current.currentAllocatedSize))
    }
}
