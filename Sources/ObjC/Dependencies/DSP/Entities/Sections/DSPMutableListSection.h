...
typedef void (^DSPMutableListCellForElement)(__kindof UITableViewCell *cell, id element, NSInteger row);
...
///
/// By default, \c kDSPDetailCell is the reuse identifier used. If you need
...
@interface DSPMutableListSection<__covariant ObjectType> : DSPCollectionContentSection

/// Initializes a section with an empty list.
+ (instancetype)list:(NSArray<ObjectType> *)list
   cellConfiguration:(DSPMutableListCellForElement)configurationBlock
...
/// A readwrite version of the same property in \c DSPTableViewSection.h
///
