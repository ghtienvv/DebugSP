...
@property (nonatomic, copy) NSArray<id<DSPShortcut>> *shortcuts;
@property (nonatomic, readonly) NSArray<id<DSPShortcut>> *allShortcuts;
@end

@implementation DSPShortcutsSection
...
+ (instancetype)forObject:(id)objectOrClass additionalRows:(NSArray *)toPrepend {
    NSArray *rows = [DSPShortcutsFactory shortcutsForObjectOrClass:objectOrClass];
...
- (id)initWithObject:object rows:(NSArray *)rows isNewSection:(BOOL)newSection {
    ...
    if (self) {
        ...
        _allShortcuts = [rows dsp_mapped:^id(id obj, NSUInteger idx) {
            return [DSPShortcut shortcutFor:obj];
        }];
        _numberOfLines = 1;
...
- (void)setFilterText:(NSString *)filterText {
    ...
    if (filterText.length) {
        ...
        [self.allTitles dsp_forEach:filterBlock];
        [self.allSubtitles dsp_forEach:filterBlock];
        ...
        self.subtitles = [self.allSubtitles dsp_filtered:^BOOL(NSString *sub, NSUInteger idx) {
            return sub.length > 0;
        }];
    }
}
...
- (void)reloadData {
    [DSPObjectExplorer configureDefaultsForItems:self.allShortcuts];
    
    // Generate all (sub)titles from shortcuts
    if (self.allShortcuts) {
        self.allTitles = [self.allShortcuts dsp_mapped:^id(id<DSPShortcut> s, NSUInteger idx) {
            return [s titleWith:self.object];
        }];
        self.allSubtitles = [self.allShortcuts dsp_mapped:^id(id<DSPShortcut> s, NSUInteger idx) {
...
- (void (^)(__kindof UIViewController *))didPressInfoButtonAction:(NSInteger)row {
    id<DSPShortcut> shortcut = self.shortcuts[row];
...
- (NSString *)reuseIdentifierForRow:(NSInteger)row {
    DSPTableViewCellReuseIdentifier defaultReuse = kDSPDetailCell;
    if (@available(iOS 11, *)) {
        defaultReuse = kDSPMultilineDetailCell;
    }
...
@interface DSPShortcutsFactory () {
    BOOL _append, _prepend, _replace, _notInstance;
    NSArray<NSString *> *_properties, *_ivars, *_methods;
}
@end

#define NewAndSet(ivar) ({ DSPShortcutsFactory *r = [self sharedFactory]; r->ivar = YES; r; })
#define SetIvar(ivar) ({ self->ivar = YES; self; })
#define SetParamBlock(ivar) ^(NSArray *p) { self->ivar = p; return self; }

typedef NSMutableDictionary<Class, NSMutableArray<id<DSPRuntimeMetadata>> *> RegistrationBuckets;

@implementation DSPShortcutsFactory {
    // Class buckets
...
+ (instancetype)sharedFactory {
    static DSPShortcutsFactory *shared = nil;
...
+ (NSArray<id<DSPRuntimeMetadata>> *)shortcutsForObjectOrClass:(id)objectOrClass {
...
- (NSArray<id<DSPRuntimeMetadata>> *)shortcutsForObjectOrClass:(id)objectOrClass {
    ...
    NSMutableArray<id<DSPRuntimeMetadata>> *shortcuts = [NSMutableArray new];
    ...
    [DSPObjectExplorer configureDefaultsForItems:shortcuts];
    return shortcuts;
}

+ (DSPShortcutsFactory *)append {
    return NewAndSet(_append);
}

+ (DSPShortcutsFactory *)prepend {
    return NewAndSet(_prepend);
}

+ (DSPShortcutsFactory *)replace {
    return NewAndSet(_replace);
}

- (void)_register:(NSArray<id<DSPRuntimeMetadata>> *)items to:(RegistrationBuckets *)global class:(Class)key {
    @synchronized (self) {
...
- (DSPShortcutsFactoryTarget)forClass {
    return ^(Class cls) {
        ...
        if (self->_properties) {
            NSArray *items = [self->_properties dsp_mapped:^id(NSString *name, NSUInteger idx) {
                return [DSPProperty named:name onClass:clsForMetadata];
            }];
            [self _register:items to:propertyBucket class:cls];
        }

        if (self->_methods) {
            NSArray *items = [self->_methods dsp_mapped:^id(NSString *name, NSUInteger idx) {
                return [DSPMethod selector:NSSelectorFromString(name) class:clsForMetadata];
            }];
        ...
        if (self->_ivars) {
            ...
            NSArray *items = [self->_ivars dsp_mapped:^id(NSString *name, NSUInteger idx) {
                return [DSPIvar named:name onClass:clsForMetadata];
            }];
