...
static NSString *DSPStringFromFIRRequestType(DSPFIRRequestType type) {
    switch (type) {
        case DSPFIRRequestTypeNotFirebase:
            return @"not firebase";
        case DSPFIRRequestTypeFetchQuery:
            return @"query fetch";
        case DSPFIRRequestTypeFetchDocument:
            return @"document fetch";
        case DSPFIRRequestTypeSetData:
            return @"set data";
        case DSPFIRRequestTypeUpdateData:
            return @"update data";
        case DSPFIRRequestTypeAddDocument:
            return @"create";
        case DSPFIRRequestTypeDeleteDocument:
            return @"delete";
    }
...
static DSPFIRTransactionDirection FIRDirectionFromRequestType(DSPFIRRequestType type) {
    switch (type) {
        case DSPFIRRequestTypeNotFirebase:
            return DSPFIRTransactionDirectionNone;
        case DSPFIRRequestTypeFetchQuery:
        case DSPFIRRequestTypeFetchDocument:
            return DSPFIRTransactionDirectionPull;
        case DSPFIRRequestTypeSetData:
        case DSPFIRRequestTypeUpdateData:
        case DSPFIRRequestTypeAddDocument:
        case DSPFIRRequestTypeDeleteDocument:
            return DSPFIRTransactionDirectionPush;
    }

    return DSPFIRTransactionDirectionNone;
}

@interface DSPFirebaseTransaction ()
@property (nonatomic) id extraData;
@property (nonatomic, readonly) NSString *queryDescription;
@end

@implementation DSPFirebaseTransaction
@synthesize queryDescription = _queryDescription;

+ (instancetype)initiator:(id)initiator requestType:(DSPFIRRequestType)type extraData:(id)data {
    DSPFirebaseTransaction *fire = [DSPFirebaseTransaction withStartTime:NSDate.date];
...
+ (instancetype)queryFetch:(FIRQuery *)initiator {
    return [self initiator:initiator requestType:DSPFIRRequestTypeFetchQuery extraData:nil];
}

+ (instancetype)documentFetch:(FIRDocumentReference *)initiator {
    return [self initiator:initiator requestType:DSPFIRRequestTypeFetchDocument extraData:nil];
}
...
+ (instancetype)setData:(FIRDocumentReference *)initiator data:(NSDictionary *)data
    ...
    DSPFirebaseSetDataInfo *info = [DSPFirebaseSetDataInfo data:data merge:merge mergeFields:mergeFields];
    return [self initiator:initiator requestType:DSPFIRRequestTypeSetData extraData:info];
}

+ (instancetype)updateData:(FIRDocumentReference *)initiator data:(NSDictionary *)data {
    return [self initiator:initiator requestType:DSPFIRRequestTypeUpdateData extraData:data];
}

+ (instancetype)addDocument:(FIRCollectionReference *)initiator document:(FIRDocumentReference *)doc {
    return [self initiator:initiator requestType:DSPFIRRequestTypeAddDocument extraData:doc];
}

+ (instancetype)deleteDocument:(FIRDocumentReference *)initiator {
    return [self initiator:initiator requestType:DSPFIRRequestTypeDeleteDocument extraData:nil];
}
...
- (DSPFirebaseSetDataInfo *)setDataInfo {
    if (self.requestType == DSPFIRRequestTypeSetData) {
...
- (NSDictionary *)updateData {
    if (self.requestType == DSPFIRRequestTypeUpdateData) {
        return self.extraData;
    }
...
- (NSString *)path {
    switch (self.direction) {
        case DSPFIRTransactionDirectionNone:
            return nil;
        case DSPFIRTransactionDirectionPush:
        case DSPFIRTransactionDirectionPull: {
...
- (NSString *)tertiaryDescription {
    if (!_tertiaryDescription) {
        ...
        [detailComponents addObject:self.direction == DSPFIRTransactionDirectionPush ?
            @"Push ↑" : @"Pull ↓"
        ];

        if (self.direction == DSPFIRTransactionDirectionPush) {
            [detailComponents addObjectsFromArray:@[DSPStringFromFIRRequestType(self.requestType)]];
        }

        if (self.state == DSPNetworkTransactionStateFinished || self.state == DSPNetworkTransactionStateFailed) {
            if (self.direction == DSPFIRTransactionDirectionPull) {
...
- (BOOL)matchesQuery:(NSString *)filterString {
    ...
    BOOL isPull = self.direction == DSPFIRTransactionDirectionPull;
    BOOL isPush = self.direction == DSPFIRTransactionDirectionPush;
