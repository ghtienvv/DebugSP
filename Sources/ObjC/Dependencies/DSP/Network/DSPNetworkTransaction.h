...
typedef NS_ENUM(NSInteger, DSPNetworkTransactionState) {
    DSPNetworkTransactionStateUnstarted = -1,
    /// This is the default; it's usually nonsense for a request to be marked as "unstarted"
    DSPNetworkTransactionStateAwaitingResponse = 0,
    DSPNetworkTransactionStateReceivingData,
    DSPNetworkTransactionStateFinished,
    DSPNetworkTransactionStateFailed
};

typedef NS_ENUM(NSUInteger, DSPWebsocketMessageDirection) {
    DSPWebsocketIncoming = 1,
    DSPWebsocketOutgoing,
};
...
@interface DSPNetworkTransaction : NSObject {
    @protected
...
@property (nonatomic) DSPNetworkTransactionState state;
...
@interface DSPURLTransaction : DSPNetworkTransaction
...
@interface DSPHTTPTransaction : DSPURLTransaction
...
@interface DSPWebsocketTransaction : DSPURLTransaction

+ (instancetype)withMessage:(NSURLSessionWebSocketMessage *)message
                       task:(NSURLSessionWebSocketTask *)task
                  direction:(DSPWebsocketMessageDirection)direction API_AVAILABLE(ios(13.0));
...
@property (nonatomic, readonly) DSPWebsocketMessageDirection direction API_AVAILABLE(ios(13.0));
...
typedef NS_ENUM(NSUInteger, DSPFIRTransactionDirection) {
    DSPFIRTransactionDirectionNone,
    DSPFIRTransactionDirectionPush,
    DSPFIRTransactionDirectionPull,
};

typedef NS_ENUM(NSUInteger, DSPFIRRequestType) {
    DSPFIRRequestTypeNotFirebase,
    DSPFIRRequestTypeFetchQuery,
    DSPFIRRequestTypeFetchDocument,
    DSPFIRRequestTypeSetData,
    DSPFIRRequestTypeUpdateData,
    DSPFIRRequestTypeAddDocument,
    DSPFIRRequestTypeDeleteDocument,
};

@interface DSPFirebaseSetDataInfo : NSObject
...
@interface DSPFirebaseTransaction : DSPNetworkTransaction
...
@property (nonatomic, readonly) DSPFIRTransactionDirection direction;
@property (nonatomic, readonly) DSPFIRRequestType requestType;
...
@property (nonatomic, readonly) DSPFirebaseSetDataInfo *setDataInfo;
