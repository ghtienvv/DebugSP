...
@interface DSPAPNSViewController ()
...
@property (nonatomic) DSPSingleRowSection *deviceToken;
@property (nonatomic) DSPMutableListSection<NSDictionary *> *remoteNotifications;
@property (nonatomic) DSPMutableListSection<UNNotification *> *userNotifications API_AVAILABLE(ios(10.0));
@end

@implementation DSPAPNSViewController
...
+ (void)load { DSP_EXIT_IF_NO_CTORS()
    if (!NSUserDefaults.standardUserDefaults.dsp_enableAPNSCapture) {
        return;
    }
...
- (NSArray<DSPTableViewSection *> *)makeSections {
    self.deviceToken = [DSPSingleRowSection title:@"APNS Device Token" reuse:nil cell:^(UITableViewCell *cell) {
        NSString *tokenString = DSPAPNSViewController.deviceTokenString;
        if (tokenString) {
            cell.textLabel.text = tokenString;
            cell.textLabel.numberOfLines = 0;
        }
        else if (!NSUserDefaults.standardUserDefaults.dsp_enableAPNSCapture) {
    ...
    self.deviceToken.selectionAction = ^(UIViewController *host) {
        UIPasteboard.generalPasteboard.string = DSPAPNSViewController.deviceTokenString;
        [DSPAlert showQuickAlert:@"Copied to Clipboard" from:host];
    };
    
    // Remote Notifications //
    
    self.remoteNotifications = [DSPMutableListSection list:DSPAPNSViewController.remoteNotifications
        cellConfiguration:^(UITableViewCell *cell, NSDictionary *notif, NSInteger row) {
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
            // TODO: date received
            cell.detailTextLabel.text = [DSPRuntimeUtility summaryForObject:notif];
        }
    ...
    self.remoteNotifications.selectionHandler = ^(UIViewController *host, NSDictionary *notif) {
        [host.navigationController pushViewController:[
            DSPObjectExplorerFactory explorerViewControllerForObject:notif
        ] animated:YES];
    };
    ...
    if (@available(iOS 10.0, *)) {
        self.userNotifications = [DSPMutableListSection list:DSPAPNSViewController.userNotifications
            cellConfiguration:^(UITableViewCell *cell, UNNotification *notif, NSInteger row) {
                cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
                
                // Subtitle is 'subtitle \n date'
                NSString *dateString = [NSDateFormatter dsp_stringFrom:notif.date format:DSPDateFormatPreciseClock];
        ...
        self.userNotifications.selectionHandler = ^(UIViewController *host, UNNotification *notif) {
            [host.navigationController pushViewController:[
                DSPObjectExplorerFactory explorerViewControllerForObject:notif.request
            ] animated:YES];
        };
...
- (void)settingsButtonTapped {
    ...
    BOOL enabled = defaults.dsp_enableAPNSCapture;
    ...
    [DSPAlert makeAlert:^(DSPAlert *make) {
        ...
        make.button(apnsToggle).destructiveStyle().handler(^(NSArray<NSString *> *strings) {
            [defaults dsp_toggleBoolForKey:kDSPDefaultsAPNSCaptureEnabledKey];
        });
