...
@interface DSPMultiColumnTableView () <
    UITableViewDataSource, UITableViewDelegate,
    UIScrollViewDelegate, DSPDBQueryRowCellLayoutSource
>
...
@implementation DSPMultiColumnTableView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.backgroundColor  = DSPColor.groupedBackgroundColor;
...
- (void)loadHeaderScrollView {
    ...
    headerScrollView.backgroundColor = DSPColor.secondaryGroupedBackgroundColor;
...
- (void)loadContentScrollView {
    ...
    [tableView registerClass:[DSPDBQueryRowCell class]
        forCellReuseIdentifier:kDSPDBQueryRowCellReuseIdentifier
    ];
...
- (void)loadLeftView {
    ...
    leftHeader.backgroundColor = DSPColor.secondaryBackgroundColor;
...
- (void)loadHeaderData {
    ...
    self.headerViews = [NSArray dsp_forEachUpTo:self.numberOfColumns map:^id(NSUInteger column) {
        DSPTableColumnHeader *header = [DSPTableColumnHeader new];
...
- (void)contentHeaderTap:(UIGestureRecognizer *)gesture {
    ...
    DSPTableColumnHeaderSortType newType = DSPNextTableColumnHeaderSortType(self.sortType);
    
    // Reset old header
    DSPTableColumnHeader *oldHeader = (id)self.headerViews[self.sortColumn];
    oldHeader.sortType = DSPTableColumnHeaderSortTypeNone;
    
    // Update new header
    DSPTableColumnHeader *newHeader = (id)self.headerViews[newSortColumn];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIColor *backgroundColor = DSPColor.primaryBackgroundColor;
    if (indexPath.row % 2 != 0) {
        backgroundColor = DSPColor.secondaryBackgroundColor;
    }
    
    // Left side table view for row numbers
    if (tableView == self.leftTableView) {
        DSPTableLeftCell *cell = [DSPTableLeftCell cellWithTableView:tableView];
    ...
    // Right side table view for data
        ...
        DSPDBQueryRowCell *cell = [tableView
            dequeueReusableCellWithIdentifier:kDSPDBQueryRowCellReuseIdentifier forIndexPath:indexPath
        ];
...
#pragma mark DSPDBQueryRowCellLayoutSource
