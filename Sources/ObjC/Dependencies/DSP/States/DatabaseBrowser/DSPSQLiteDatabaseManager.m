...
@interface DSPSQLiteDatabaseManager ()
@property (nonatomic) sqlite3 *db;
@property (nonatomic, copy) NSString *path;
@end

@implementation DSPSQLiteDatabaseManager

#pragma mark - DSPDatabaseManager
...
#if SQLITE_HAS_CODEC
    NSString *defaultSqliteDatabasePassword = DSPManager.sharedManager.defaultSqliteDatabasePassword;
...
- (NSArray<NSString *> *)queryAllTables {
    return [[self executeStatement:QUERY_TABLENAMES].rows dsp_mapped:^id(NSArray *table, NSUInteger idx) {
...
- (NSArray<NSString *> *)queryAllColumnsOfTable:(NSString *)tableName {
    ...
    DSPSQLResult *results = [self executeStatement:sql];
    
    // https://github.com/DSPool/DSP/issues/554
    ...
    return [results.keyedRows dsp_mapped:^id(NSDictionary *column, NSUInteger idx) {
        return column[@"name"];
    }] ?: @[];
}
...
- (NSArray<NSString *> *)queryRowIDsInTable:(NSString *)tableName {
    ...
    return [data dsp_mapped:^id(NSArray<NSString *> *obj, NSUInteger idx) {
        return obj.firstObject;
    }];
}

- (DSPSQLResult *)executeStatement:(NSString *)sql {
    return [self executeStatement:sql arguments:nil];
}

- (DSPSQLResult *)executeStatement:(NSString *)sql arguments:(NSDictionary *)args {
    [self open];
    
    DSPSQLResult *result = nil;
    ...
    if ((status = sqlite3_prepare_v2(_db, sql.UTF8String, -1, &pstmt, 0)) == SQLITE_OK) {
        ...
        NSArray<NSString *> *columns = [NSArray dsp_forEachUpTo:columnCount map:^id(NSUInteger i) {
        ...
        while ((status = sqlite3_step(pstmt)) == SQLITE_ROW) {
            ...
            if (dataCount > 0) {
                [rows addObject:[NSArray dsp_forEachUpTo:columnCount map:^id(NSUInteger i) {
        ...
        if (status == SQLITE_DONE) {
            ...
            if (rows.count || columnCount > 0) {
                ...
                result = _lastResult = [DSPSQLResult columns:columns rows:rows];
            } else {
                // We executed a query like INSERT, UPDATE, or DELETE
                int rowsAffected = sqlite3_changes(_db);
                NSString *message = [NSString stringWithFormat:@"%d row(s) affected", rowsAffected];
                result = _lastResult = [DSPSQLResult message:message];
            }
        } else {
