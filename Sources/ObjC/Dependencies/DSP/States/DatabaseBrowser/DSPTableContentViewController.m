...
@interface DSPTableContentViewController () <
    DSPMultiColumnTableViewDataSource, DSPMultiColumnTableViewDelegate
>
...
@property (nonatomic, readonly, nullable) id<DSPDatabaseManager> databaseManager;

@property (nonatomic, readonly) BOOL canRefresh;

@property (nonatomic) DSPMultiColumnTableView *multiColumnView;
@end

@implementation DSPTableContentViewController
...
- (instancetype)initWithColumns:(NSArray<NSString *> *)columnNames
                      ...
                      tableName:(nullable NSString *)tableName
                       database:(nullable id<DSPDatabaseManager>)databaseManager {
...
- (DSPMultiColumnTableView *)multiColumnView {
    if (!_multiColumnView) {
        _multiColumnView = [[DSPMultiColumnTableView alloc]
            initWithFrame:DSPRectSetSize(CGRectZero, self.view.frame.size)
        ];
...
- (CGFloat)multiColumnTableView:(DSPMultiColumnTableView *)tableView
...
- (void)multiColumnTableView:(DSPMultiColumnTableView *)tableView didSelectRow:(NSInteger)row {
    NSArray<NSString *> *fields = [self.rows[row] dsp_mapped:^id(NSString *field, NSUInteger idx) {
        return [NSString stringWithFormat:@"%@:\n%@", self.columns[idx], field];
    }];
    
    NSArray<NSString *> *values = [self.rows[row] dsp_mapped:^id(NSString *value, NSUInteger idx) {
    ...
    [DSPAlert makeAlert:^(DSPAlert *make) {
        ...
        make.button(@"Focus on Row").handler(^(NSArray<NSString *> *strings) {
            UIViewController *focusedRow = [DSPTableRowDataViewController
...
- (void)multiColumnTableView:(DSPMultiColumnTableView *)tableView
    didSelectHeaderForColumn:(NSInteger)column
                    sortType:(DSPTableColumnHeaderSortType)sortType {
    ...
    if (sortType == DSPTableColumnHeaderSortTypeDesc) {
...
- (void)setupToolbarItems {
    ...
    UIBarButtonItem *trashButton = DSPBarButtonItemSystem(Trash, self, @selector(trashPressed));
    UIBarButtonItem *addButton = DSPBarButtonItemSystem(Add, self, @selector(addPressed));
    ...
    self.toolbarItems = @[
        ...
        addButton,
        UIBarButtonItem.dsp_dspibleSpace,
        [trashButton dsp_withTintColor:UIColor.redColor],
    ];
}
...
- (void)executeStatementAndShowResult:(NSString *)statement
    ...
    DSPSQLResult *result = [self.databaseManager executeStatement:statement];
