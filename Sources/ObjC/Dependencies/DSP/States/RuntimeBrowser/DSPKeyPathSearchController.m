...
@interface DSPKeyPathSearchController ()
@property (nonatomic, readonly, weak) id<DSPKeyPathSearchControllerDelegate> delegate;
...
@property (nonatomic) DSPRuntimeKeyPath *keyPath;
...
@property (nonatomic) NSArray<NSArray<DSPMethod *> *> *classesToMethods;
@end

@implementation DSPKeyPathSearchController

+ (instancetype)delegate:(id<DSPKeyPathSearchControllerDelegate>)delegate {
    DSPKeyPathSearchController *controller = [self new];
    controller->_bundlesOrClasses = [DSPRuntimeController allBundleNames];
...
- (void)setToolbar:(DSPRuntimeBrowserToolbar *)toolbar {
    _toolbar = toolbar;
...
- (void)didSelectKeyPathOption:(NSString *)text {
    ...
    NSString *keyPath = [orig dsp_stringByReplacingLastKeyPathComponent:text];
    self.delegate.searchController.searchBar.text = keyPath;

    self.keyPath = [DSPRuntimeKeyPathTokenizer tokenizeString:keyPath];
...
- (void)didPressButton:(NSString *)text insertInto:(UISearchBar *)searchBar {
    ...
    if ([self searchBar:searchBar shouldChangeTextInRange:field.dsp_selectedRange replacementText:text]) {
...
- (NSArray<NSString *> *)suggestions {
    if (self.bundlesOrClasses) {
        if (self.classes) {
            ...
            return [self.filteredClasses dsp_subArrayUpto:10];
        }
        ...
        return [self.bundlesOrClasses dsp_subArrayUpto:10];
    }
...
- (void)updateTable {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        if (self.classes) {
            ...
            NSMutableArray *methods = [DSPRuntimeController
            ...
            DSPRuntimeKeyPath *keyPath = self.keyPath;
            NSArray *models = [DSPRuntimeController dataForKeyPath:keyPath];
            if (keyPath.methodKey) { // We're looking at methods
                self.bundlesOrClasses = nil;
                
                NSMutableArray *methods = models.mutableCopy;
                NSMutableArray<NSString *> *classes = [
                    DSPRuntimeController classesForKeyPath:keyPath
                ];
...
- (void)setNonEmptyMethodLists:(NSMutableArray<NSArray<DSPMethod *> *> *)methods
...
- (BOOL)searchBar:(UISearchBar *)searchBar shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    ...
    if (![DSPRuntimeKeyPathTokenizer allowedInKeyPath:text]) {
        return NO;
    }
    ...
    @try {
        ...
        self.keyPath = [DSPRuntimeKeyPathTokenizer tokenizeString:text];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    ...
    if (searchText.length) {
        ...
        self.timer = [NSTimer dsp_fireSecondsFromNow:0.15 block:^{
            [self updateTable];
        }];
    }
    // ... or remove all rows
    else {
        _bundlesOrClasses = [DSPRuntimeController allBundleNames];
...
- (void)searchBarCancelButtonClicked:(UISearchBar *)searchBar {
    self.keyPath = DSPRuntimeKeyPath.empty;
    [self updateTable];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView
        dequeueReusableCellWithIdentifier:kDSPMultilineDetailCell
    ...
    // One row per section
        ...
        NSArray<DSPMethod *> *methods = self.classesToMethods[indexPath.row];
        NSMutableString *summary = [NSMutableString new];
        [methods enumerateObjectsUsingBlock:^(DSPMethod *method, NSUInteger idx, BOOL *stop) {
            NSString *format = nil;
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    if (self.filteredClasses || self.keyPath.methodKey) {
        ...
        if (self.keyPath.classKey) {
            return DSPPluralString(count, @"classes", @"class");
        } else {
            return DSPPluralString(count, @"bundles", @"bundle");
        }
    }
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    NSString *bundleSuffixOrClass = self.bundlesOrClasses[indexPath.row];
    NSString *imagePath = [DSPRuntimeController imagePathWithShortName:bundleSuffixOrClass];
