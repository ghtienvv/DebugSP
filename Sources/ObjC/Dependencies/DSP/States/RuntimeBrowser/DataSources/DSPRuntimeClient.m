...
@implementation DSPRuntimeClient

#pragma mark - Initialization

+ (instancetype)runtime {
    static DSPRuntimeClient *runtime;
...
- (void)reloadLibrariesList {
    ...
    if (imageNames) {
        NSMutableArray *imageNameStrings = [NSMutableArray dsp_forEachUpTo:imageCount map:^NSString *(NSUInteger i) {
            return @(imageNames[i]);
        }];
        ...
        _imageDisplayNames = [imageNameStrings dsp_mapped:^id(NSString *path, NSUInteger idx) {
...
- (NSMutableArray<NSString *> *)classNamesInImageAtPath:(NSString *)path {
    ...
    if (classNames) {
        classNameStrings = [NSMutableArray dsp_forEachUpTo:classCount map:^id(NSUInteger i) {
            return @(classNames[i]);
        }];
...
- (NSArray<Class> *)copySafeClassList {
    ...
    return [NSArray dsp_forEachUpTo:count map:^id(NSUInteger i) {
        Class cls = classes[i];
        return DSPClassIsSafe(cls) ? cls : nil;
    }];
}
...
- (NSMutableArray<NSString *> *)bundleNamesForToken:(DSPSearchToken *)token {
    if (self.imagePaths.count) {
        ...
        return [_imageDisplayNames dsp_mapped:^id(NSString *binary, NSUInteger idx) {
...
- (NSMutableArray<NSString *> *)bundlePathsForToken:(DSPSearchToken *)token {
    if (self.imagePaths.count) {
        ...
        return [self.imagePaths dsp_mapped:^id(NSString *binary, NSUInteger idx) {
            NSString *UIName = [self shortNameForImageName:binary];
...
- (NSMutableArray<NSString *> *)classesForToken:(DSPSearchToken *)token inBundles:(NSMutableArray<NSString *> *)bundles {
    // Edge case where token is the class we want already; return superclasses
    if (token.isAbsolute) {
        if (DSPClassIsSafe(NSClassFromString(token.string))) {
    ...
    if (bundles.count) {
        ...
        return [names dsp_mapped:^NSString *(NSString *name, NSUInteger idx) {
            Class cls = NSClassFromString(name);
            BOOL safe = DSPClassIsSafe(cls);
...
- (NSMutableArray<NSString *> *)_classesForToken:(DSPSearchToken *)token inBundles:(NSMutableArray<NSString *> *)bundles {
    TBWildcardOptions options = token.options;
    NSString *query = token.string;
    ...
    if (bundles.count == 1) {
        ...
        return [[self classNamesInImageAtPath:bundles.firstObject] dsp_mapped:^id(NSString *className, NSUInteger idx) {
        ...
        // Optimization, avoid a loop
        if (options == TBWildcardOptionsAny) {
            return [[bundles dsp_flatmapped:^NSArray *(NSString *bundlePath, NSUInteger idx) {
                return [self classNamesInImageAtPath:bundlePath];
            }] dsp_sortedUsingSelector:@selector(caseInsensitiveCompare:)];
        }

        return [[bundles dsp_flatmapped:^NSArray *(NSString *bundlePath, NSUInteger idx) {
            return [[self classNamesInImageAtPath:bundlePath] dsp_mapped:^id(NSString *className, NSUInteger idx) {
                return TBWildcardMap(query, className, options);
            }];
        }] dsp_sortedUsingSelector:@selector(caseInsensitiveCompare:)];
    }
    ...
    if (classes.count) {
        ...
        switch (options) {
            ...
            case TBWildcardOptionsNone: {
                ...
                return @[[classes dsp_mapped:^id(NSString *name, NSUInteger idx) {
                    Class cls = NSClassFromString(name);
                    ...
                    // Method is absolute
                    return [DSPMethod selector:sel class:cls];
                }]];
            }
            case TBWildcardOptionsAny: {
                return [classes dsp_mapped:^NSArray *(NSString *name, NSUInteger idx) {
                    // Any means `instance` was not specified
                    Class cls = NSClassFromString(name);
                    return [cls dsp_allMethods];
                }];
            }
            default: {
                // Only "if contains"
                if (options & TBWildcardOptionsPrefix &&
                    ...
                    return [classes dsp_mapped:^NSArray *(NSString *name, NSUInteger idx) {
                        Class cls = NSClassFromString(name);
                        return [[cls dsp_allMethods] dsp_mapped:^id(DSPMethod *method, NSUInteger idx) {
                ...
                // Only "if method starts with with selector"
                    ...
                    return [classes dsp_mapped:^NSArray *(NSString *name, NSUInteger idx) {
                        ...
                        if (!selector.length) {
                            if (instance) {
                                return [cls dsp_allInstanceMethods];
                            } else {
                                return [cls dsp_allClassMethods];
                            }
                        }

                        id mapping = ^id(DSPMethod *method) {
                        ...
                        if (instance) {
                            return [[cls dsp_allInstanceMethods] dsp_mapped:mapping];
                        } else {
                            return [[cls dsp_allClassMethods] dsp_mapped:mapping];
                        }
                    }];
                }
            }
