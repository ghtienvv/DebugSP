...
@interface DSPRuntimeExporter ()
@property (nonatomic, readonly) DSPSQLiteDatabaseManager *db;
...
@property (nonatomic, copy) NSArray<DSPProtocol *> *protocols;
...
@implementation DSPRuntimeExporter

+ (NSString *)tempFilename {
    NSString *temp = NSTemporaryDirectory();
    NSString *uuid = [NSUUID.UUID.UUIDString substringToIndex:8];
    NSString *filename = [NSString stringWithFormat:@"DSPRuntimeDatabase-%@.db", uuid];
...
+ (void)createRuntimeDatabaseAtPath:(NSString *)path
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        DSPRuntimeExporter *exporter = [self new];
...
- (BOOL)createAndPopulateDatabaseAtPath:(NSString *)path
    ...
    _db = [DSPSQLiteDatabaseManager managerForDatabase:path];
...
- (void)loadMetadata:(void(^)(NSString *status))progress {
    ...
    DSPRuntimeClient *runtime = DSPRuntimeClient.runtime;
    
    // Only load metadata for the existing paths if any
    if (self.loadedBundlePaths) {
        // Images
        self.loadedShortBundleNames = [self.loadedBundlePaths dsp_mapped:^id(NSString *path, NSUInteger idx) {
            return [runtime shortNameForImageName:path];
        }];
        
        // Classes
        self.classes = [[runtime classesForToken:DSPSearchToken.any
            inBundles:self.loadedBundlePaths.mutableCopy
        ] dsp_mapped:^id(NSString *cls, NSUInteger idx) {
            return NSClassFromString(cls);
        }];
    } else {
        // Images
        self.loadedShortBundleNames = runtime.imageDisplayNames;
        self.loadedBundlePaths = [self.loadedShortBundleNames dsp_mapped:^id(NSString *name, NSUInteger idx) {
    ...
    self.protocols = [[runtime copyProtocolList] dsp_mapped:^id(Protocol *proto, NSUInteger idx) {
        return [DSPProtocol protocol:proto];
    }];
}
...
- (BOOL)addImages:(void(^)(NSString *status))progress {
    ...
    DSPSQLiteDatabaseManager *database = self.db;
...
NS_INLINE BOOL FREInsertProtocolMember(DSPSQLiteDatabaseManager *db,
...
- (BOOL)addProtocols:(void(^)(NSString *status))progress {
    ...
    for (DSPProtocol *proto in self.protocols) {
        ...
        // Required methods
        for (DSPMethodDescription *method in proto.requiredMethods) {
        ...
        // Optional methods
        for (DSPMethodDescription *method in proto.optionalMethods) {
            NSString *selector = NSStringFromSelector(method.selector);
        ...
        if (@available(iOS 10, *)) {
            // Required properties
            for (DSPProperty *property in proto.requiredProperties) {
            ...
            // Optional properties
            for (DSPProperty *property in proto.optionalProperties) {
                BOOL success = FREInsertProtocolMember(
            ...
            // Just... properties.
            for (DSPProperty *property in proto.properties) {
                BOOL success = FREInsertProtocolMember(
...
- (BOOL)addProtocolConformances:(void(^)(NSString *status))progress {
    ...
    for (DSPProtocol *proto in self.protocols) {
        ...
        for (DSPProtocol *conform in proto.protocols) {
...
- (BOOL)addClassConformances:(void(^)(NSString *status))progress {
    ...
    for (Class cls in self.classes) {
        ...
        for (DSPProtocol *conform in DSPGetConformedProtocols(cls)) {
...
- (BOOL)addIvars:(void(^)(NSString *status))progress {
    ...
    for (Class cls in self.classes) {
        for (DSPIvar *ivar in DSPGetAllIvars(cls)) {
            // Insert type first
...
- (BOOL)addMethods:(void(^)(NSString *status))progress {
    ...
    for (Class cls in self.classes) {
        ...
        BOOL (^insert)(DSPMethod *, NSNumber *) = ^BOOL(DSPMethod *method, NSNumber *instance) {
        ...
        for (DSPMethod *method in DSPGetAllMethods(cls, YES)) {
            if (!insert(method, @YES)) {
                return NO;
            }
        }
        for (DSPMethod *method in DSPGetAllMethods(object_getClass(cls), NO)) {
            if (!insert(method, @NO)) {
                return NO;
            }
        }
...
- (BOOL)addProperties:(void(^)(NSString *status))progress {
    ...
    for (Class cls in self.classes) {
        ...
        // Block used to process each message
        BOOL (^insert)(DSPProperty *, NSNumber *) = ^BOOL(DSPProperty *property, NSNumber *instance) {
            DSPPropertyAttributes *attrs = property.attributes;
            ...
            NSInteger size = [DSPTypeEncodingParser
        ...
        for (DSPProperty *property in DSPGetAllProperties(cls)) {
            if (!insert(property, @YES)) {
                return NO;
            }
        }
        for (DSPProperty *property in DSPGetAllProperties(object_getClass(cls))) {
...
- (BOOL)addMethodSignature:(DSPMethod *)method {
...
- (BOOL)executeInsert:(NSString *)statement
    ...
    // Insert
    DSPSQLiteDatabaseManager *database = _db;
