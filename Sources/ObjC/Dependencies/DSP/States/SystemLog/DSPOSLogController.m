...
@interface DSPOSLogController ()

+ (DSPOSLogController *)sharedLogController;

@property (nonatomic) void (^updateHandler)(NSArray<DSPSystemLogMessage *> *);
...
@implementation DSPOSLogController

+ (void)load {
    // Persist logs when the app launches on iOS 10 if we have persistent logs turned on
    if (DSPOSLogAvailable()) {
        if (NSUserDefaults.standardUserDefaults.dsp_cacheOSLogMessages) {
...
+ (instancetype)sharedLogController {
    static DSPOSLogController *shared = nil;
...
+ (instancetype)withUpdateHandler:(void(^)(NSArray<DSPSystemLogMessage *> *newMessages))newMessagesHandler {
    DSPOSLogController *shared = [self sharedLogController];
    shared.updateHandler = newMessagesHandler;
    return shared;
}

- (id)init {
    NSAssert(DSPOSLogAvailable(), @"os_log is only available on iOS 10 and up");
...
- (BOOL)handleStreamEntry:(os_activity_stream_entry_t)entry error:(int)error {
    ...
    if (!error && entry) {
        if (entry->type == OS_ACTIVITY_STREAM_TYPE_LOG_MESSAGE ||
            ...
            // https://github.com/DSPool/DSP/issues/564
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                DSPSystemLogMessage *message = [DSPSystemLogMessage logMessageFromDate:date text:msg];
                if (self.persistent) {
