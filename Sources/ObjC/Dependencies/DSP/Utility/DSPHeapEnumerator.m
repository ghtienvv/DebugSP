...
@implementation DSPHeapEnumerator
...
static void range_callback(task_t task, void *context, unsigned type, vm_range_t *ranges, unsigned rangeCount) {
    ...
    for (unsigned int i = 0; i < rangeCount; i++) {
        ...
        dsp_maybe_object_t *tryObject = (dsp_maybe_object_t *)range.address;
        Class tryClass = NULL;
#ifdef __arm64__
        ...
        if (CFSetContainsValue(registeredClasses, (__bridge const void *)(tryClass))) {
            (*(dsp_object_enumeration_block_t __unsafe_unretained *)context)((__bridge id)tryObject, tryClass);
        }
    }
}
...
+ (void)enumerateLiveObjectsUsingBlock:(dsp_object_enumeration_block_t)block {
    if (!block) {
        return;
    }
    ...
    if (result == KERN_SUCCESS) {
        for (unsigned int i = 0; i < zoneCount; i++) {
            ...
            dsp_object_enumeration_block_t callback = ^(__unsafe_unretained id object, __unsafe_unretained Class actualClass) {
                unlock_zone(zone);
                block(object, actualClass);
                lock_zone(zone);
            };
            
            BOOL lockZoneValid = DSPPointerIsReadable(lock_zone);
            BOOL unlockZoneValid =  DSPPointerIsReadable(unlock_zone);
...
+ (NSArray<DSPObjectRef *> *)instancesOfClassWithName:(NSString *)className retained:(BOOL)retain {
    ...
    [DSPHeapEnumerator enumerateLiveObjectsUsingBlock:^(__unsafe_unretained id object, __unsafe_unretained Class actualClass) {
    ...
    NSArray<DSPObjectRef *> *references = [DSPObjectRef referencingAll:instances retained:retain];
    return references;
}

+ (NSArray<DSPObjectRef *> *)objectsWithReferencesToObject:(id)object retained:(BOOL)retain {
    NSMutableArray<DSPObjectRef *> *instances = [NSMutableArray new];
    [DSPHeapEnumerator enumerateLiveObjectsUsingBlock:^(__unsafe_unretained id tryObject, __unsafe_unretained Class actualClass) {
        // Skip known-invalid objects
        if (!DSPPointerIsValidObjcObject((__bridge void *)tryObject)) {
            return;
        }
        ...
        while (tryClass) {
            ...
            for (unsigned int ivarIndex = 0; ivarIndex < ivarCount; ivarIndex++) {
                ...
                if (typeEncoding.dsp_typeIsObjectOrClass) {
                    ptrdiff_t offset = ivar_getOffset(ivar);
                    uintptr_t *fieldPointer = (__bridge void *)tryObject + offset;

                    if (*fieldPointer == (uintptr_t)(__bridge void *)object) {
                        NSString *ivarName = @(ivar_getName(ivar) ?: "???");
                        id ref = [DSPObjectRef referencing:tryObject ivar:ivarName retained:retain];
...
+ (DSPHeapSnapshot *)generateHeapSnapshot {
    ...
    [DSPHeapEnumerator enumerateLiveObjectsUsingBlock:^(__unsafe_unretained id object, __unsafe_unretained Class cls) {
    ...
    return [DSPHeapSnapshot snapshotWithCounts:countsForClassNames sizes:sizesForClassNames];
}
