...
NSString * const DSPRuntimeUtilityErrorDomain = @"DSPRuntimeUtilityErrorDomain";

@implementation DSPRuntimeUtility

#pragma mark - General Helpers (Public)

+ (BOOL)pointerIsValidObjcObject:(const void *)pointer {
    return DSPPointerIsValidObjcObject(pointer);
}
...
+ (id)potentiallyUnwrapBoxedPointer:(id)returnedObjectOrNil type:(const DSPTypeEncoding *)returnType {
    ...
    NSInteger i = 0;
    if (returnType[i] == DSPTypeEncodingConst) {
        i++;
    }

    BOOL returnsObjectOrClass = returnType[i] == DSPTypeEncodingObjcObject ||
                                returnType[i] == DSPTypeEncodingObjcClass;
    BOOL returnsVoidPointer   = returnType[i] == DSPTypeEncodingPointer &&
                                returnType[i+1] == DSPTypeEncodingVoid;
    BOOL returnsCString       = returnType[i] == DSPTypeEncodingCString;
    ...
    if (!returnsObjectOrClass) {
        ...
        if (returnsCString) {
            ...
            if ([DSPRuntimeUtility pointerIsValidObjcObject:value.pointerValue]) {
...
+ (NSUInteger)fieldNameOffsetForTypeEncoding:(const DSPTypeEncoding *)typeEncoding {
    ...
    while (typeEncoding[beginIndex] == DSPTypeEncodingQuote) {
        NSUInteger endIndex = beginIndex + 1;
        while (typeEncoding[endIndex] != DSPTypeEncodingQuote) {
            ++endIndex;
        }
...
+ (NSArray<DSPObjectRef *> *)subclassesOfClassWithName:(NSString *)className {
    NSArray<Class> *classes = DSPGetAllSubclasses(NSClassFromString(className), NO);
    NSArray<DSPObjectRef *> *references = [DSPObjectRef referencingClasses:classes];
    return references;
}
...
+ (NSArray<NSString *> *)allPropertyAttributeKeys {
    ...
    dispatch_once(&onceToken, ^{
        allPropertyAttributeKeys = @[
            kDSPPropertyAttributeKeyTypeEncoding,
            kDSPPropertyAttributeKeyBackingIvarName,
            kDSPPropertyAttributeKeyReadOnly,
            kDSPPropertyAttributeKeyCopy,
            kDSPPropertyAttributeKeyRetain,
            kDSPPropertyAttributeKeyNonAtomic,
            kDSPPropertyAttributeKeyCustomGetter,
            kDSPPropertyAttributeKeyCustomSetter,
            kDSPPropertyAttributeKeyDynamic,
            kDSPPropertyAttributeKeyWeak,
            kDSPPropertyAttributeKeyGarbageCollectable,
...
+ (NSArray<NSString *> *)prettyArgumentComponentsForMethod:(Method)method {
    ...
    for (unsigned int argIndex = 0; argIndex < selectorComponents.count; argIndex++) {
        char *argType = method_copyArgumentType(method, argIndex + kDSPNumberOfImplicitArgs);
...
+ (id)performSelector:(SEL)selector
    ...
    if (mightForwardMsgSend || ![self safeObject:object respondsToSelector:selector]) {
        if (error) {
            ...
            *error = [NSError
                errorWithDomain:DSPRuntimeUtilityErrorDomain
                code:DSPRuntimeUtilityErrorCodeDoesNotRecognizeSelector
                userInfo:userInfo
            ];
        }
    ...
    NSMethodSignature *methodSignature = [DSPMethod selector:selector class:cls].signature;
    if (!methodSignature) {
    ...
    if (!methodSignature.methodReturnLength &&
        methodSignature.methodReturnType[0] != DSPTypeEncodingVoid) {
        return nil;
    }
    ...
    for (NSUInteger argumentIndex = kDSPNumberOfImplicitArgs; argumentIndex < numberOfArguments; argumentIndex++) {
        NSUInteger argumentsArrayIndex = argumentIndex - kDSPNumberOfImplicitArgs;
        ...
        if (argumentObject && ![argumentObject isKindOfClass:[NSNull class]]) {
            ...
            if (typeEncodingCString[0] == DSPTypeEncodingObjcObject ||
              typeEncodingCString[0] == DSPTypeEncodingObjcClass ||
              [self isTollFreeBridgedValue:argumentObject forCFType:typeEncodingCString]) {
                ...
                if (strcmp([argumentValue objCType], typeEncodingCString) != 0) {
                    if (error) {
                        ...
                        *error = [NSError
                            errorWithDomain:DSPRuntimeUtilityErrorDomain
                            code:DSPRuntimeUtilityErrorCodeArgumentTypeMismatch
                            userInfo:userInfo
                        ];
                    }
                    return nil;
                ...
                @try {
                    ...
                    DSPGetSizeAndAlignment(typeEncodingCString, &bufferSize, NULL);
    ...
    @try {
        ...
        if (returnType[0] == DSPTypeEncodingObjcObject || returnType[0] == DSPTypeEncodingObjcClass) {
        ...
        } else if (returnType[0] != DSPTypeEncodingVoid) {
            NSAssert(methodSignature.methodReturnLength, @"Memory corruption lies ahead");

            if (returnType[0] == DSPTypeEncodingStructBegin) {
                if (selector == stdStringExclusion && [object isKindOfClass:[NSString class]]) {
                    ...
                    if (error) {
                        *error = [NSError
                            errorWithDomain:DSPRuntimeUtilityErrorDomain
                            code:DSPRuntimeUtilityErrorCodeInvocationFailed
...
+ (NSValue *)valueForNumberWithObjCType:(const char *)typeEncoding fromInputString:(NSString *)inputString {
    ...
    if (strlen(typeEncoding) > 1) {
        ...
        if ([type isEqualToString:@DSPEncodeClass(NSDecimalNumber)]) {
            return [NSDecimalNumber decimalNumberWithString:inputString];
        } else if ([type isEqualToString:@DSPEncodeClass(NSNumber)]) {
            return number;
        }
...
+ (void)enumerateTypesInStructEncoding:(const char *)structEncoding
    ...
    if (structEncoding && structEncoding[0] == DSPTypeEncodingStructBegin) {
        ...
        if (equals) {
            ...
            if (DSPGetSizeAndAlignment(structEncoding, &structSize, &fieldAlignment)) {
                NSUInteger runningFieldIndex = 0;
                NSUInteger runningFieldOffset = 0;
                const char *typeStart = equals + 1;
                
                while (*typeStart != DSPTypeEncodingStructEnd) {
                    NSUInteger fieldSize = 0;
                    // If the struct type encoding was successfully handled by
                    // DSPGetSizeAndAlignment above, we *should* be ok with the field here.
...
+ (NSString *)appendName:(NSString *)name toType:(NSString *)type {
    ...
    if ([type characterAtIndex:type.length - 1] == DSPTypeEncodingCString) {
...
+ (NSString *)readableTypeForEncoding:(NSString *)encodingString {
    ...
    const NSUInteger fieldNameOffset = [DSPRuntimeUtility fieldNameOffsetForTypeEncoding:encodingCString];
    if (fieldNameOffset > 0) {
        ...
        NSArray<NSString *> *const fieldNames = [fieldNamesString
            componentsSeparatedByString:[NSString stringWithFormat:@"%c", DSPTypeEncodingQuote]
        ];
    ...
    // Objects
    if (encodingCString[0] == DSPTypeEncodingObjcObject) {
        NSString *class = [encodingString substringFromIndex:1];
        class = [class stringByReplacingOccurrencesOfString:@"\"" withString:@""];
        if (class.length == 0 || (class.length == 1 && [class characterAtIndex:0] == DSPTypeEncodingUnknown)) {
            class = @"id";
        } else {
...
#undef TRANSLATE
    ...
    if (encodingCString[0] == DSPTypeEncodingStructBegin) {
        ...
        if (equals) {
            ...
            if (nameStart[0] == DSPTypeEncodingUnknown) {
...
+ (NSValue *)valueForPrimitivePointer:(void *)pointer objCType:(const char *)type {
    ...
    const NSUInteger fieldNameOffset = [DSPRuntimeUtility fieldNameOffsetForTypeEncoding:type];
    if (fieldNameOffset > 0) {
...
#undef CASE
    ...
    NSValue *value = nil;
    if (DSPGetSizeAndAlignment(type, nil, nil)) {
        @try {
