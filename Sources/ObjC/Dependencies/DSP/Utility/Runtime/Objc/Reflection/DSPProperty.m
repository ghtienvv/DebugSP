...
- (id)initWithProperty:(objc_property_t)property onClass:(Class)cls {
    ...
    if (self) {
        ...
        _attributes    = [DSPPropertyAttributes attributesForProperty:property];
...
- (id)initWithName:(NSString *)name attributes:(DSPPropertyAttributes *)attributes {
...
- (void)examine {
    if (self.attributes.typeEncoding.length) {
        _type = (DSPTypeEncoding)[self.attributes.typeEncoding characterAtIndex:0];
    }
...
- (NSString *)description {
    if (!_dsp_description) {
        NSString *readableType = [DSPRuntimeUtility readableTypeForEncoding:self.attributes.typeEncoding];
...
- (NSString *)fullDescription {
    ...
    DSPPropertyAttributes *attributes = self.attributes;
...
- (id)getValue:(id)target {
    ...
    if (self.likelyGetterExists) {
        ...
        if (instanceAndInstanceProperty || classAndClassProperty) {
            return [DSPRuntimeUtility performSelector:self.likelyGetter onObject:target];
        }
    }
...
- (DSPMethodBase *)getterWithImplementation:(IMP)implementation {
    ...
    DSPMethodBase *getter = [DSPMethodBase buildMethodNamed:name withTypes:types implementation:implementation];
    return getter;
}
...
- (DSPMethodBase *)setterWithImplementation:(IMP)implementation {
    ...
    DSPMethodBase *setter = [DSPMethodBase buildMethodNamed:name withTypes:types implementation:implementation];
    return setter;
}
