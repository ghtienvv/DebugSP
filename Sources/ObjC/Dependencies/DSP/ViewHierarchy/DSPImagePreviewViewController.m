...
@interface DSPImagePreviewViewController () <UIScrollViewDelegate>
...
#pragma mark -
@implementation DSPImagePreviewViewController

#pragma mark Initialization

+ (instancetype)previewForView:(UIView *)view {
    return [self forImage:[DSPUtility previewImageForView:view]];
}

+ (instancetype)previewForLayer:(CALayer *)layer {
    return [self forImage:[DSPUtility previewImageForLayer:layer]];
}
...
- (id)initWithImage:(UIImage *)image {
    ...
    if (self) {
        self.title = @"Preview";
        self.image = image;
        _backgroundColors = @[DSPResources.checkerPatternColor, UIColor.whiteColor, UIColor.blackColor];
    }
...
- (void)actionButtonPressed:(id)sender {
    ...
    UIViewController *activityVC = [DSPActivityViewController sharing:@[self.image] source:sender];
