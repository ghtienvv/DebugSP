...
- (id)initWithView:(UIView *)view isInScrollView:(BOOL)inScrollView {
    ...
    if (self) {
        ...
        UIViewController *controller = [DSPUtility viewControllerForView:view];
        if (controller) {
            _important = YES;
...
- (NSArray<FHSView *> *)children {
    ...
    return [self.view.subviews dsp_mapped:^id(UIView *subview, NSUInteger idx) {
...
+ (UIImage *)_snapshotView:(UIView *)view {
    ...
    [view.subviews dsp_forEach:^(UIView *v, NSUInteger idx) {
        if (!v.isHidden) {
