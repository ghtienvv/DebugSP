...
@interface DSPHierarchyTableViewCell ()
...
@implementation DSPHierarchyTableViewCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        self.depthIndicatorView.backgroundColor = DSPUtility.hierarchyIndentPatternColor;
        [self.contentView addSubview:self.depthIndicatorView];
        
        UIImage *defaultCircleImage = [DSPUtility circularImageWithColor:UIColor.blackColor radius:5];
        ...
        self.textLabel.font = UIFont.dsp_defaultTableCellFont;
        self.detailTextLabel.font = UIFont.dsp_defaultTableCellFont;
        ...
        dispatch_once(&once, ^{
            checkerPatternColor = [UIColor colorWithPatternImage:DSPResources.checkerPattern];
        });
        
        self.backgroundColorCheckerPatternView = [UIView new];
        self.backgroundColorCheckerPatternView.clipsToBounds = YES;
        self.backgroundColorCheckerPatternView.layer.borderColor = DSPColor.tertiaryBackgroundColor.CGColor;
...
- (void)layoutSubviews {
    ...
    circleFrame.origin.y = DSPFloor(textLabelCenterY - CGRectGetHeight(circleFrame) / 2.f);
...
- (void)setRandomColorTag:(UIColor *)randomColorTag {
    if (![_randomColorTag isEqual:randomColorTag]) {
        ...
        self.colorCircleImageView.image = [DSPUtility circularImageWithColor:randomColorTag radius:6];
    }
}
