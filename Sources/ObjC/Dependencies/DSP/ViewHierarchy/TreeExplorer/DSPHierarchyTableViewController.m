...
#import "DSPUtility.h"
#import "DSPHierarchyTableViewCell.h"
#import "DSPObjectExplorerViewController.h"
#import "DSPObjectExplorerFactory.h"
#import "DSPResources.h"
#import "DSPWindow.h"

typedef NS_ENUM(NSUInteger, DSPHierarchyScope) {
    DSPHierarchyScopeFullHierarchy,
    DSPHierarchyScopeViewsAtTap
};

@interface DSPHierarchyTableViewController ()
...
@implementation DSPHierarchyTableViewController
...
- (void)viewDidLoad {
    ...
    if (self.showScopeBar) {
        ...
        self.selectedScope = DSPHierarchyScopeViewsAtTap;
    }
...
+ (NSArray<UIView *> *)allViewsInHierarchy:(NSArray<UIWindow *> *)windows {
    return [windows dsp_flatmapped:^id(UIWindow *window, NSUInteger idx) {
        if (![window isKindOfClass:[DSPWindow class]]) {
...
- (void)updateDisplayedViews {
    ...
    if (self.showScopeBar) {
        if (self.selectedScope == DSPHierarchyScopeViewsAtTap) {
            candidateViews = self.viewsAtTap;
        } else if (self.selectedScope == DSPHierarchyScopeFullHierarchy) {
    ...
    if (self.searchText.length) {
        self.displayedViews = [candidateViews filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(UIView *candidateView, NSDictionary<NSString *, id> *bindings) {
            NSString *title = [DSPUtility descriptionForView:candidateView includingFrame:NO];
...
static NSString * const kDSPHierarchyTableViewCellReuseIdentifier = @"Cell";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DSPHierarchyTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kDSPHierarchyTableViewCellReuseIdentifier];
    if (!cell) {
        cell = [[DSPHierarchyTableViewCell alloc] initWithReuseIdentifier:kDSPHierarchyTableViewCellReuseIdentifier];
    }
    
    UIView *view = self.displayedViews[indexPath.row];

    cell.textLabel.text = [DSPUtility descriptionForView:view includingFrame:NO];
    cell.detailTextLabel.text = [DSPUtility detailDescriptionForView:view];
    cell.randomColorTag = [DSPUtility consistentRandomColorForObject:view];
    ...
    if (view.isHidden || view.alpha < 0.01) {
        cell.textLabel.textColor = DSPColor.deemphasizedTextColor;
        cell.detailTextLabel.textColor = DSPColor.deemphasizedTextColor;
    } else {
        cell.textLabel.textColor = DSPColor.primaryTextColor;
        cell.detailTextLabel.textColor = DSPColor.primaryTextColor;
    }
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
    ...
    DSPObjectExplorerViewController *viewExplorer = [DSPObjectExplorerFactory explorerViewControllerForObject:drillInView];
