...
@interface DSPFilteringTableViewController ()

@end

@implementation DSPFilteringTableViewController
...
- (void)_registerCellsForReuse {
    for (DSPTableViewSection *section in self.filterDelegate.allSections) {
...
- (void)setFilterDelegate:(id<DSPTableViewFiltering>)filterDelegate {
...
- (NSArray<DSPTableViewSection *> *)nonemptySections {
    return [self.filterDelegate.allSections dsp_filtered:^BOOL(DSPTableViewSection *section, NSUInteger idx) {
...
- (void)setAllSections:(NSArray<DSPTableViewSection *> *)allSections {
    _allSections = allSections.copy;
    // Only display nonempty sections
    self.sections = self.nonemptySections;
}

- (void)setSections:(NSArray<DSPTableViewSection *> *)sections {
    // Allow sections to reload a portion of the table view at will
    [sections enumerateObjectsUsingBlock:^(DSPTableViewSection *s, NSUInteger idx, BOOL *stop) {
...
- (NSArray<NSString *> *)sectionIndexTitlesForTableView:(UITableView *)tableView {
    if (self.wantsSectionIndexTitles) {
        return [NSArray dsp_forEachUpTo:self.filterDelegate.sections.count map:^id(NSUInteger i) {
            return @"⦁";
        }];
    }
