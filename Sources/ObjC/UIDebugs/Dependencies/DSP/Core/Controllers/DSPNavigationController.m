...
@interface DSPNavigationController ()
...
@implementation DSPNavigationController

+ (instancetype)withRootViewController:(UIViewController *)rootVC {
    DSPNavigationController *nav = [[self alloc] initWithRootViewController:rootVC];
    return nav;
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (self.waitingToAddTab) {
        ...
        if ([self.presentingViewController isKindOfClass:[DSPExplorerViewController class]]) {
            // New navigation controllers always add themselves as new tabs,
            // tabs are closed by DSPExplorerViewController
            [DSPTabList.sharedList addTab:self];
...
@implementation UINavigationController (DSPObjectExploring)
