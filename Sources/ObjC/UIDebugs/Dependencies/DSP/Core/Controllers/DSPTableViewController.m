...
CGFloat const kDSPDebounceInstant = 0.f;
CGFloat const kDSPDebounceFast = 0.05;
CGFloat const kDSPDebounceForAsyncSearch = 0.15;
CGFloat const kDSPDebounceForExpensiveIO = 0.5;

@interface DSPTableViewController ()
...
@implementation DSPTableViewController
@dynamic tableView;
...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        _searchBarDebounceInterval = kDSPDebounceFast;
...
- (void)setShowsCarousel:(BOOL)showsCarousel {
    ...
    if (showsCarousel) {
        _carousel = ({ weakify(self)
            ...
            DSPScopeCarousel *carousel = [DSPScopeCarousel new];
            ...
            [carousel registerBlockForDynamicTypeChanges:^(DSPScopeCarousel *_) { strongify(self);
...
- (void)loadView {
    self.view = [DSPTableView style:self.style];
    self.tableView.dataSource = self;
    self.tableView.delegate = self;
    self.tableView.estimatedRowHeight = 10;
    
    _shareToolbarItem = DSPBarButtonItemSystem(Action, self, @selector(shareButtonPressed:));
    _bookmarksToolbarItem = [UIBarButtonItem
        dsp_itemWithImage:DSPResources.bookmarksIcon target:self action:@selector(showBookmarks)
    ];
    _openTabsToolbarItem = [UIBarButtonItem
        dsp_itemWithImage:DSPResources.openTabsIcon target:self action:@selector(showTabSwitcher)
    ];
    
    self.leftmostToolbarItem = UIBarButtonItem.dsp_fixedSpace;
    self.middleLeftToolbarItem = UIBarButtonItem.dsp_fixedSpace;
    self.middleToolbarItem = UIBarButtonItem.dsp_fixedSpace;
}
...
- (void)setupToolbarItems {
    ...
    // Disable tabs entirely when not presented by DSPExplorerViewController
    UIViewController *presenter = self.navigationController.presentingViewController;
    if (![presenter isKindOfClass:[DSPExplorerViewController class]]) {
...
- (void)layoutTableHeaderIfNeeded {
    if (self.showsCarousel) {
        self.carousel.frame = DSPRectSetHeight(
...
- (void)addCarousel:(DSPScopeCarousel *)carousel {
    if (@available(iOS 11.0, *)) {
        ...
        if (self.showsSearchBar) {
            carousel.frame = subviewFrame = DSPRectSetY(
...
- (void)removeCarousel:(DSPScopeCarousel *)carousel {
...
- (void)addSearchController:(UISearchController *)controller {
    if (@available(iOS 11.0, *)) {
        ...
        if (self.showsCarousel) {
            self.carousel.frame = DSPRectSetY(
...
- (void)removeSearchController:(UISearchController *)controller {
    if (@available(iOS 11.0, *)) {
        ...
        if (self.showsCarousel) {
//            self.carousel.frame = DSPRectRemake(CGPointZero, self.carousel.frame.size);
...
- (void)showBookmarks {
    UINavigationController *nav = [[UINavigationController alloc]
        initWithRootViewController:[DSPBookmarksViewController new]
    ];
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController {
    ...
    if (text.length && self.searchBarDebounceInterval > kDSPDebounceInstant) {
