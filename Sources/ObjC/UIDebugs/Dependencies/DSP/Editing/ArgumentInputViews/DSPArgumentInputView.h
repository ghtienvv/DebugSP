...
typedef NS_ENUM(NSUInteger, DSPArgumentInputViewSize) {
    /// 2 lines, medium-sized
    DSPArgumentInputViewSizeDefault = 0,
    /// One line
    DSPArgumentInputViewSizeSmall,
    /// Several lines
    DSPArgumentInputViewSizeLarge
};

@protocol DSPArgumentInputViewDelegate;

@interface DSPArgumentInputView : UIView
...
@property (nonatomic) DSPArgumentInputViewSize targetSize;

/// Users of the input view can get delegate callbacks for incremental changes in user input.
@property (nonatomic, weak) id <DSPArgumentInputViewDelegate> delegate;
...
/// Used by DSPArgumentInputViewFactory to create appropriate input views.
...
@protocol DSPArgumentInputViewDelegate <NSObject>
