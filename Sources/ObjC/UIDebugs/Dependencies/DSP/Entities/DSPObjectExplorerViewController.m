...
@interface DSPObjectExplorerViewController () <UIGestureRecognizerDelegate>
@property (nonatomic, readonly) DSPSingleRowSection *descriptionSection;
@property (nonatomic, readonly) NSArray<DSPTableViewSection *> *customSections;
...
@implementation DSPObjectExplorerViewController

#pragma mark - Initialization

+ (instancetype)exploringObject:(id)target {
    return [self exploringObject:target customSection:[DSPShortcutsSection forObject:target]];
}

+ (instancetype)exploringObject:(id)target customSection:(DSPTableViewSection *)section {
...
+ (instancetype)exploringObject:(id)target customSections:(NSArray *)customSections {
    return [[self alloc]
        ...
        explorer:[DSPObjectExplorer forObject:target]
        customSections:customSections
    ];
}

- (id)initWithObject:(id)target
            explorer:(__kindof DSPObjectExplorer *)explorer
       customSections:(NSArray<DSPTableViewSection *> *)customSections {
...
- (NSArray<NSString *> *)observedNotifications {
    return @[
        kDSPDefaultsHidePropertyIvarsKey,
        kDSPDefaultsHidePropertyMethodsKey,
        kDSPDefaultsHidePrivateMethodsKey,
        kDSPDefaultsShowMethodOverridesKey,
        kDSPDefaultsHideVariablePreviewsKey,
    ];
}
...
- (void)viewDidLoad {
    ...
    self.title = [DSPRuntimeUtility safeClassNameForObject:self.object];
    ...
    self.carousel.items = [self.explorer.classHierarchyClasses dsp_mapped:^id(Class cls, NSUInteger idx) {
        return NSStringFromClass(cls);
    }];
    
    // ... button for extra options
    [self addToolbarItems:@[[UIBarButtonItem
        dsp_itemWithImage:DSPResources.moreIcon target:self action:@selector(moreButtonPressed:)
    ]]];
...
- (NSArray<DSPTableViewSection *> *)nonemptySections {
    ...
    return [super.nonemptySections dsp_filtered:^BOOL(DSPTableViewSection *section, NSUInteger idx) {
        return section != self.descriptionSection;
    }];
}

- (NSArray<DSPTableViewSection *> *)makeSections {
    DSPObjectExplorer *explorer = self.explorer;
    
    // Description section is only for instances
    if (self.explorer.objectIsInstance) {
        _descriptionSection = [DSPSingleRowSection
            title:@"Description" reuse:kDSPMultilineCell cell:^(DSPTableViewCell *cell) {
                cell.titleLabel.font = UIFont.dsp_defaultTableCellFont;
    ...
    DSPSingleRowSection *referencesSection = [DSPSingleRowSection
        title:@"Object Graph" reuse:kDSPDefaultCell cell:^(DSPTableViewCell *cell) {
    ...
    referencesSection.selectionAction = ^(UIViewController *host) {
        UIViewController *references = [DSPObjectListViewController
    ...
    NSMutableArray *sections = [NSMutableArray arrayWithArray:@[
        [DSPMetadataSection explorer:self.explorer kind:DSPMetadataKindProperties],
        [DSPMetadataSection explorer:self.explorer kind:DSPMetadataKindClassProperties],
        [DSPMetadataSection explorer:self.explorer kind:DSPMetadataKindIvars],
        [DSPMetadataSection explorer:self.explorer kind:DSPMetadataKindMethods],
        [DSPMetadataSection explorer:self.explorer kind:DSPMetadataKindClassMethods],
        [DSPMetadataSection explorer:self.explorer kind:DSPMetadataKindClassHierarchy],
        [DSPMetadataSection explorer:self.explorer kind:DSPMetadataKindProtocols],
        [DSPMetadataSection explorer:self.explorer kind:DSPMetadataKindOther],
        referencesSection
    ]];
...
- (void)shareButtonPressed:(UIBarButtonItem *)sender {
    [DSPAlert makeSheet:^(DSPAlert *make) {
        make.button(@"Add to Bookmarks").handler(^(NSArray<NSString *> *strings) {
            [DSPBookmarkManager.bookmarks addObject:self.object];
        });
        ...
        make.button(@"Copy Address").handler(^(NSArray<NSString *> *strings) {
            UIPasteboard.generalPasteboard.string = [DSPUtility addressOfObject:self.object];
        });
...
- (void)moreButtonPressed:(UIBarButtonItem *)sender {
    ...
    NSDictionary<NSString *, NSString *> *explorerToggles = @{
        kDSPDefaultsHidePropertyIvarsKey:    @"Property-Backing Ivars",
        kDSPDefaultsHidePropertyMethodsKey:  @"Property-Backing Methods",
        kDSPDefaultsHidePrivateMethodsKey:   @"Likely Private Methods",
        kDSPDefaultsShowMethodOverridesKey:  @"Method Overrides",
        kDSPDefaultsHideVariablePreviewsKey: @"Variable Previews"
    };
    ...
    NSDictionary<NSString *, NSDictionary *> *nextStateDescriptions = @{
        kDSPDefaultsHidePropertyIvarsKey:    @{ @NO: @"Hide ", @YES: @"Show " },
        kDSPDefaultsHidePropertyMethodsKey:  @{ @NO: @"Hide ", @YES: @"Show " },
        kDSPDefaultsHidePrivateMethodsKey:   @{ @NO: @"Hide ", @YES: @"Show " },
        kDSPDefaultsShowMethodOverridesKey:  @{ @NO: @"Show ", @YES: @"Hide " },
        kDSPDefaultsHideVariablePreviewsKey: @{ @NO: @"Hide ", @YES: @"Show " },
    };
    ...
    [DSPAlert makeSheet:^(DSPAlert *make) {
        ...
        for (NSString *option in explorerToggles.allKeys) {
            ...
            make.button(title).handler(^(NSArray<NSString *> *strings) {
                [NSUserDefaults.standardUserDefaults dsp_toggleBoolForKey:option];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    DSPTableViewSection *section = self.filterDelegate.sections[indexPath.section];
    
    if (section == self.descriptionSection) {
        NSAttributedString *attributedText = [[NSAttributedString alloc]
            initWithString:self.explorer.objectDescription
            attributes:@{ NSFontAttributeName : UIFont.dsp_defaultTableCellFont }
        ];
