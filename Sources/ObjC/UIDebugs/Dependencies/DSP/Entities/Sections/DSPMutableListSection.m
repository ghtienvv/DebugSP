...
@interface DSPMutableListSection ()
@property (nonatomic, readonly) DSPMutableListCellForElement configureCell;
@end

@implementation DSPMutableListSection
@synthesize cellRegistrationMapping = _cellRegistrationMapping;

#pragma mark - Initialization

+ (instancetype)list:(NSArray *)list
   cellConfiguration:(DSPMutableListCellForElement)cellConfig
...
configurationBlock:(DSPMutableListCellForElement)cellConfig
     filterMatcher:(BOOL(^)(NSString *, id))filterBlock {
    self = [super init];
    if (self) {
