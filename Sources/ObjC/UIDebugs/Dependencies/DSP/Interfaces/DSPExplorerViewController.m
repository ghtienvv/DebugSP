...
#import "DSPWindowManagerController.h"
#import "DSPViewControllersViewController.h"
#import "NSUserDefaults+DSP.h"

typedef NS_ENUM(NSUInteger, DSPExplorerMode) {
    DSPExplorerModeDefault,
    DSPExplorerModeSelect,
    DSPExplorerModeMove
};

@interface DSPExplorerViewController () <DSPHierarchyDelegate, UIAdaptivePresentationControllerDelegate>

/// Tracks the currently active tool/mode
@property (nonatomic) DSPExplorerMode currentMode;
...
/// self.view.window as a \c DSPWindow
@property (nonatomic, readonly) DSPWindow *window;
...
@implementation DSPExplorerViewController
...
- (void)viewDidLoad {
    ...
    // Toolbar
    _explorerToolbar = [DSPExplorerToolbar new];

    // Start the toolbar off below any bars that may be at the top of the view.
    CGFloat toolbarOriginY = NSUserDefaults.standardUserDefaults.dsp_toolbarTopMargin;
    ...
    self.movePanGR.enabled = self.currentMode == DSPExplorerModeMove;
...
- (UIViewController *)viewControllerForRotationAndOrientation {
    UIViewController *viewController = DSPUtility.appKeyWindow.rootViewController;
...
//    }
    ...
    UIInterfaceOrientationMask supportedOrientations = DSPUtility.infoPlistSupportedInterfaceOrientationsMask;
    ...
    // recursively when I'm inspecting DSP with itself from a tweak dylib
    if (viewControllerToAsk && ![NSStringFromClass([viewControllerToAsk class]) hasPrefix:@"DSP"]) {
...
- (void)viewWillTransitionToSize:(CGSize)size
    ...
    [coordinator animateAlongsideTransition:^(id<UIViewControllerTransitionCoordinatorContext> context) {
        ...
        for (UIView *view in self.viewsAtTapPoint) {
            ...
            if (self.currentMode == DSPExplorerModeSelect) {
...
- (void)setSelectedView:(UIView *)selectedView {
    if (![_selectedView isEqual:selectedView]) {
        ...
        self.explorerToolbar.selectedViewDescription = [DSPUtility
            descriptionForView:selectedView includingFrame:YES
        ];
        self.explorerToolbar.selectedViewOverlayColor = [DSPUtility
        ...
        if (selectedView) {
            ...
            UIColor *outlineColor = [DSPUtility consistentRandomColorForObject:selectedView];
...
- (void)setCurrentMode:(DSPExplorerMode)currentMode {
    if (_currentMode != currentMode) {
        _currentMode = currentMode;
        switch (currentMode) {
            case DSPExplorerModeDefault:
                [self removeAndClearOutlineViews];
                self.viewsAtTapPoint = nil;
                self.selectedView = nil;
                break;
                
            case DSPExplorerModeSelect:
        ...
        self.movePanGR.enabled = currentMode == DSPExplorerModeMove;
        [self updateButtonStates];
    }
}
...
- (void)setupToolbarActions {
    DSPExplorerToolbar *toolbar = self.explorerToolbar;
    NSDictionary<NSString *, DSPExplorerToolbarItem *> *actionsToItems = @{
    ...
    [actionsToItems enumerateKeysAndObjectsUsingBlock:^(NSString *sel, DSPExplorerToolbarItem *item, BOOL *stop) {
        [item addTarget:self action:NSSelectorFromString(sel) forControlEvents:UIControlEventTouchUpInside];
    }];
}

- (void)selectButtonTapped:(DSPExplorerToolbarItem *)sender {
    [self toggleSelectTool];
}

- (void)hierarchyButtonTapped:(DSPExplorerToolbarItem *)sender {
...
- (void)recentButtonTapped:(DSPExplorerToolbarItem *)sender {
    NSAssert(DSPTabList.sharedList.activeTab, @"Must have active tab");
    [self presentViewController:DSPTabList.sharedList.activeTab animated:YES completion:nil];
}

- (void)moveButtonTapped:(DSPExplorerToolbarItem *)sender {
    [self toggleMoveTool];
}

- (void)globalsButtonTapped:(DSPExplorerToolbarItem *)sender {
    [self toggleMenuTool];
}

- (void)closeButtonTapped:(DSPExplorerToolbarItem *)sender {
    self.currentMode = DSPExplorerModeDefault;
...
- (void)updateButtonStates {
    ...
    toolbar.selectItem.selected = self.currentMode == DSPExplorerModeSelect;
    ...
    toolbar.moveItem.selected = self.currentMode == DSPExplorerModeMove;
    
    // Recent only enabled when we have a last active tab
    if (!self.presentedViewController) {
        toolbar.recentItem.enabled = DSPTabList.sharedList.activeTab != nil;
    } else {
...
- (void)updateToolbarPositionWithUnconstrainedFrame:(CGRect)unconstrainedFrame {
    ...
    NSUserDefaults.standardUserDefaults.dsp_toolbarTopMargin = unconstrainedFrame.origin.y;
}
...
- (void)handleToolbarDetailsTapGesture:(UITapGestureRecognizer *)tapGR {
    if (tapGR.state == UIGestureRecognizerStateRecognized && self.selectedView) {
        UIViewController *topStackVC = [DSPObjectExplorerFactory explorerViewControllerForObject:self.selectedView];
        [self presentViewController:
            [DSPNavigationController withRootViewController:topStackVC]
...
- (void)handleToolbarShowTabsGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        // Don't use DSPNavigationController because the tab viewer itself is not a tab
        [super presentViewController:[[UINavigationController alloc]
            initWithRootViewController:[DSPTabsViewController new]
        ] animated:YES completion:nil];
    }
}
...
- (void)handleToolbarWindowManagerGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan) {
        ...
        [super presentViewController:[DSPNavigationController
            withRootViewController:[DSPWindowManagerController new]
        ] animated:YES completion:nil];
    }
}
...
- (void)handleToolbarShowViewControllersGesture:(UILongPressGestureRecognizer *)sender {
    if (sender.state == UIGestureRecognizerStateBegan && self.viewsAtTapPoint.count) {
        ...
        UIViewController *list = [DSPViewControllersViewController
            controllersForViews:self.viewsAtTapPoint
        ];
        [self presentViewController:
            [DSPNavigationController withRootViewController:list
        ] animated:YES completion:nil];
    }
}
...
- (void)handleSelectionTap:(UITapGestureRecognizer *)tapGR {
    ...
    if (self.currentMode == DSPExplorerModeSelect && tapGR.state == UIGestureRecognizerStateRecognized) {
        // Note that [tapGR locationInView:nil] is broken in iOS 8,
        // so we have to do a two step conversion to window coordinates.
        // Thanks to @lascorbe for finding this: https://github.com/Flipboard/DSP/pull/31
...
- (UIView *)outlineViewForView:(UIView *)view {
    ...
    outlineView.layer.borderColor = [DSPUtility consistentRandomColorForObject:view].CGColor;
...
- (NSArray<UIView *> *)viewsAtPoint:(CGPoint)tapPointInWindow skipHiddenViews:(BOOL)skipHidden {
    ...
    for (UIWindow *window in DSPUtility.allWindows) {
...
- (UIView *)viewForSelectionAtPoint:(CGPoint)tapPointInWindow {
    ...
    for (UIWindow *window in DSPUtility.allWindows.reverseObjectEnumerator) {
...
- (void)updateSelectedViewPositionWithDragGesture:(UIPanGestureRecognizer *)movePanGR {
    ...
    newSelectedViewFrame.origin.x = DSPFloor(newSelectedViewFrame.origin.x + translation.x);
    newSelectedViewFrame.origin.y = DSPFloor(newSelectedViewFrame.origin.y + translation.y);
...
- (BOOL)shouldReceiveTouchAtWindowPoint:(CGPoint)pointInWindowCoordinates {
    ...
    if (self.currentMode == DSPExplorerModeMove) {
        return YES;
    }
...
#pragma mark - DSPHierarchyDelegate
...
- (void)viewHierarchyDidDismiss:(UIView *)selectedView {
    ...
    [self toggleViewsToolWithCompletion:^{
        ...
        if (self.currentMode == DSPExplorerModeDefault && selectedView) {
            self.currentMode = DSPExplorerModeSelect;
        }
...
- (void)presentViewController:(UIViewController *)toPresent
    ...
    // Move the status bar on top of DSP so we can get scroll to top behavior for taps.
    if (!@available(iOS 13, *)) {
...
- (void)dismissViewControllerAnimated:(BOOL)animated completion:(void (^)(void))completion {    
    ...
    // We want it above DSP while a modal is presented for
    // scroll to top, but below DSP otherwise for exploration.
...
- (void)toggleMoveTool {
    if (self.currentMode == DSPExplorerModeMove) {
        self.currentMode = DSPExplorerModeSelect;
    } else if (self.currentMode == DSPExplorerModeSelect && self.selectedView) {
        self.currentMode = DSPExplorerModeMove;
    }
}
...
- (void)toggleViewsToolWithCompletion:(void(^)(void))completion {
    [self toggleToolWithViewControllerProvider:^UINavigationController *{
        if (self.selectedView) {
            return [DSPHierarchyViewController
                delegate:self
                viewsAtTap:self.viewsAtTapPoint
                selectedView:self.selectedView
            ];
        } else {
            return [DSPHierarchyViewController delegate:self];
        }
...
- (void)toggleMenuTool {
    [self toggleToolWithViewControllerProvider:^UINavigationController *{
        return [DSPNavigationController withRootViewController:[DSPGlobalsViewController new]];
    } completion:nil];
}
...
- (BOOL)handleDownArrowKeyPressed {
    ...
    } else if (self.currentMode == DSPExplorerModeSelect && self.viewsAtTapPoint.count > 0) {
