...
@interface DSPWindowManagerController ()
...
@implementation DSPWindowManagerController
...
- (void)reloadData {
    ...
    self.windowSubtitles = [self.windows dsp_mapped:^id(UIWindow *window, NSUInteger idx) {
    ...
    if (@available(iOS 13, *)) {
        ...
        self.sceneSubtitles = [self.scenes dsp_mapped:^id(UIScene *scene, NSUInteger idx) {
...
- (void)showRevertOrDismissAlert:(void(^)(void))revertBlock {
    ...
    } showFrom:[DSPUtility topViewControllerInWindow:highestWindow]];
}
...
- (NSString *)sceneDescription:(UIScene *)scene API_AVAILABLE(ios(13)) {
    ...
    if ([scene isKindOfClass:[UIWindowScene class]]) {
        ...
        suffix = DSPPluralString(windowScene.windows.count, @"windows", @"window");
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    UIWindow *window = nil;
    NSString *subtitle = nil;
    DSPWindow *dsp = DSPManager.sharedManager.explorerWindow;
    ...
    switch (indexPath.section) {
        ...
        case 2:
            if (@available(iOS 13, *)) {
                UIScene *scene = self.scenes[indexPath.row];
                UIWindowScene *oldScene = dsp.windowScene;
                BOOL isWindowScene = [scene isKindOfClass:[UIWindowScene class]];
                BOOL isDSPScene = isWindowScene ? dsp.windowScene == scene : NO;
                ...
                [DSPAlert makeAlert:^(DSPAlert *make) {
                    ...
                    if (isWindowScene) {
                        if (isDSPScene) {
                            make.message(@"Already the DSP window scene");
                        }
                        
                        make.button(@"Set as DSP Window Scene")
                        .handler(^(NSArray<NSString *> *strings) {
                            dsp.windowScene = (id)scene;
                            [self showRevertOrDismissAlert:^{
                                dsp.windowScene = oldScene;
                            }];
                        }).enabled(!isDSPScene);
    ...
    subtitle = [subtitle stringByAppendingString:
        @"\n\n1) Adjust the DSP window level relative to this window,\n"
        "2) adjust this window's level relative to the DSP window,\n"
    ...
    [DSPAlert makeAlert:^(DSPAlert *make) {
        ...
        make.button(@"Adjust DSP Window Level").handler(^(NSArray<NSString *> *strings) {
            targetWindow = dsp; oldLevel = dsp.windowLevel;
            dsp.windowLevel = window.windowLevel + strings.firstObject.integerValue;
        ...
        make.button(@"Adjust This Window's Level").handler(^(NSArray<NSString *> *strings) {
            ...
            window.windowLevel = dsp.windowLevel + strings.firstObject.integerValue;
...
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)ip {
    [self.navigationController pushViewController:
        [DSPObjectExplorerFactory explorerViewControllerForObject:self.sections[ip.section][ip.row]]
    animated:YES];
}
