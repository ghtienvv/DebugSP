...
@interface DSPTabList () {
    NSMutableArray *_openTabs;
    NSMutableArray *_openTabSnapshots;
}
@end
#pragma mark -
@implementation DSPTabList

#pragma mark Initialization

+ (DSPTabList *)sharedList {
    static DSPTabList *sharedList = nil;
...
- (void)addTab:(UINavigationController *)newTab {
    ...
    [_openTabSnapshots addObject:[DSPUtility previewImageForView:newTab.view]];
    _activeTab = newTab;
...
- (void)updateSnapshotForActiveTab {
    if (self.activeTabIndex != NSNotFound) {
        UIImage *newSnapshot = [DSPUtility previewImageForView:self.activeTab.view];
