...
#import "DSPNetworkTransactionCell.h"
#import "DSPHTTPTransactionDetailController.h"
#import "DSPNetworkSettingsController.h"
...
typedef NS_ENUM(NSInteger, DSPNetworkObserverMode) {
    DSPNetworkObserverModeFirebase = 0,
    DSPNetworkObserverModeREST,
    DSPNetworkObserverModeWebsockets,
};

@interface DSPNetworkMITMViewController ()

@property (nonatomic) BOOL updateInProgress;
@property (nonatomic) BOOL pendingReload;

@property (nonatomic) DSPNetworkObserverMode mode;

@property (nonatomic, readonly) DSPMITMDataSource<DSPNetworkTransaction *> *dataSource;
@property (nonatomic, readonly) DSPMITMDataSource<DSPHTTPTransaction *> *HTTPDataSource;
@property (nonatomic, readonly) DSPMITMDataSource<DSPWebsocketTransaction *> *websocketDataSource;
@property (nonatomic, readonly) DSPMITMDataSource<DSPFirebaseTransaction *> *firebaseDataSource;

@end

@implementation DSPNetworkMITMViewController
...
- (void)viewDidLoad {
    ...
    _HTTPDataSource = [DSPMITMDataSource dataSourceWithProvider:^NSArray * {
        return DSPNetworkRecorder.defaultRecorder.HTTPTransactions;
    }];

    if (kFirebaseAvailable) {
        _firebaseDataSource = [DSPMITMDataSource dataSourceWithProvider:^NSArray * {
            return DSPNetworkRecorder.defaultRecorder.firebaseTransactions;
        }];
    ...
    if (kWebsocketsAvailable) {
        ...
        _websocketDataSource = [DSPMITMDataSource dataSourceWithProvider:^NSArray * {
            return DSPNetworkRecorder.defaultRecorder.websocketTransactions;
        }];
    }
    ...
    self.mode = NSUserDefaults.standardUserDefaults.dsp_lastNetworkObserverMode;
    ...
    [self addToolbarItems:@[
        ...
        [[UIBarButtonItem
          dsp_systemItem:UIBarButtonSystemItemTrash
          target:self
          action:@selector(trashButtonTapped:)
        ] dsp_withTintColor:UIColor.redColor]
    ]];

    [self.tableView
        registerClass:DSPNetworkTransactionCell.class
        forCellReuseIdentifier:DSPNetworkTransactionCell.reuseID
    ];
    self.tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    self.tableView.rowHeight = DSPNetworkTransactionCell.preferredCellHeight;
...
- (void)settingsButtonTapped:(UIBarButtonItem *)sender {
    UIViewController *settings = [DSPNetworkSettingsController new];
    settings.navigationItem.rightBarButtonItem = DSPBarButtonItemSystem(
        Done, self, @selector(settingsViewControllerDoneTapped:)
    );
    settings.title = @"Network Debugging Settings";
    
    // This is not a DSPNavigationController because it is not intended as a new tab
...
- (void)trashButtonTapped:(UIBarButtonItem *)sender {
    [DSPAlert makeSheet:^(DSPAlert *make) {
        ...
        make.button(@"Clear").destructiveStyle().handler(^(NSArray *strings) {
            if (clearAll) {
                [DSPNetworkRecorder.defaultRecorder clearRecordedActivity];
            } else {
                DSPNetworkTransactionKind kind = (DSPNetworkTransactionKind)self.mode;
                [DSPNetworkRecorder.defaultRecorder clearRecordedActivity:kind matching:self.searchText];
            }
        });
...
- (DSPNetworkObserverMode)mode {
    DSPNetworkObserverMode mode = self.searchController.searchBar.selectedScopeButtonIndex;
    switch (mode) {
        case DSPNetworkObserverModeFirebase:
            if (kFirebaseAvailable) {
                return DSPNetworkObserverModeFirebase;
            }

            return DSPNetworkObserverModeREST;
        case DSPNetworkObserverModeREST:
            if (kFirebaseAvailable) {
                return DSPNetworkObserverModeREST;
            }

            return DSPNetworkObserverModeWebsockets;
        case DSPNetworkObserverModeWebsockets:
            return DSPNetworkObserverModeWebsockets;
    }
}

- (void)setMode:(DSPNetworkObserverMode)mode {
...
// is unavailable, DSPNetworkObserverModeFirebase represents the same index
...
// if it is unavailable, we subtract 1 and it becomes DSPNetworkObserverModeREST.
...
- (DSPMITMDataSource<DSPNetworkTransaction *> *)dataSource {
    switch (self.mode) {
        case DSPNetworkObserverModeREST:
...
- (void)updateTransactions:(void(^)(void))callback {
    id completion = ^(DSPMITMDataSource *dataSource) {
        // Update byte count
...
- (NSString *)headerText {
    ...
    if (self.mode == DSPNetworkObserverModeFirebase) {
...
+ (DSPGlobalsEntryRowAction)globalsEntryRowAction:(DSPGlobalsRow)row {
    return ^(UITableViewController *host) {
        if (DSPNetworkObserver.isEnabled) {
...
- (void)tryUpdateTransactions {
    ...
    DSPNetworkObserverMode currentMode = self.mode;
    ...
    [self updateTransactions:^{
        ...
        DSPNetworkObserverMode newMode = self.mode;
...
- (void)handleTransactionUpdatedNotification:(NSNotification *)notification {
    ...
    DSPNetworkTransaction *transaction = notification.userInfo[kDSPNetworkRecorderUserInfoTransactionKey];

    // Update both the main table view and search table view if needed.
    for (DSPNetworkTransactionCell *cell in self.tableView.visibleCells) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    DSPNetworkTransactionCell *cell = [tableView
        dequeueReusableCellWithIdentifier:DSPNetworkTransactionCell.reuseID
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (self.mode) {
        case DSPNetworkObserverModeREST: {
            DSPHTTPTransaction *transaction = [self HTTPTransactionAtIndexPath:indexPath];
            UIViewController *details = [DSPHTTPTransactionDetailController withTransaction:transaction];
        ...
        case DSPNetworkObserverModeWebsockets: {
            if (@available(iOS 13.0, *)) { // This check will never fail
                DSPWebsocketTransaction *transaction = [self websocketTransactionAtIndexPath:indexPath];
                
                UIViewController *details = nil;
                if (transaction.message.type == NSURLSessionWebSocketMessageTypeData) {
                    details = [DSPObjectExplorerFactory explorerViewControllerForObject:transaction.message.data];
                } else {
                    details = [[DSPWebViewController alloc] initWithText:transaction.message.string];
                }
        ...
        case DSPNetworkObserverModeFirebase: {
            DSPFirebaseTransaction *transaction = [self firebaseTransactionAtIndexPath:indexPath];
//            id obj = transaction.documents.count == 1 ? transaction.documents.firstObject : transaction.documents;
            UIViewController *explorer = [DSPObjectExplorerFactory explorerViewControllerForObject:transaction];
...
- (UIContextMenuConfiguration *)tableView:(UITableView *)tableView contextMenuConfigurationForRowAtIndexPath:(NSIndexPath *)indexPath point:(CGPoint)point __IOS_AVAILABLE(13.0) {
    ...
    DSPNetworkTransaction *transaction = [self transactionAtIndexPath:indexPath];
    ...
    return [UIContextMenuConfiguration
        ...
        actionProvider:^UIMenu *(NSArray<UIMenuElement *> *suggestedActions) {
            ...
            if (self.mode == DSPNetworkObserverModeREST) {
                ...
                UIAction *denylist = [UIAction
                    ...
                    identifier:nil
                    handler:^(__kindof UIAction *action) {
                        NSMutableArray *denylist =  DSPNetworkRecorder.defaultRecorder.hostDenylist;
                        [denylist addObject:request.URL.host];
                        [DSPNetworkRecorder.defaultRecorder clearExcludedTransactions];
                        [DSPNetworkRecorder.defaultRecorder synchronizeDenylist];
                        [self tryUpdateTransactions];
                    }
                ];
...
- (DSPNetworkTransaction *)transactionAtIndexPath:(NSIndexPath *)indexPath {
    return self.dataSource.transactions[indexPath.row];
}

- (DSPHTTPTransaction *)HTTPTransactionAtIndexPath:(NSIndexPath *)indexPath {
    return self.HTTPDataSource.transactions[indexPath.row];
}

- (DSPWebsocketTransaction *)websocketTransactionAtIndexPath:(NSIndexPath *)indexPath {
    return self.websocketDataSource.transactions[indexPath.row];
}

- (DSPFirebaseTransaction *)firebaseTransactionAtIndexPath:(NSIndexPath *)indexPath {
...
- (void)updateSearchResults:(NSString *)searchString {
    id callback = ^(DSPMITMDataSource *dataSource) {
...
- (void)searchBar:(UISearchBar *)searchBar selectedScopeButtonIndexDidChange:(NSInteger)newScope {
    ...
    NSUserDefaults.standardUserDefaults.dsp_lastNetworkObserverMode = self.mode;
}
