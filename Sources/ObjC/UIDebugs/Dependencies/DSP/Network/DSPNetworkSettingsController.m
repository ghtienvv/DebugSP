...
@interface DSPNetworkSettingsController () <UIActionSheetDelegate>
...
@implementation DSPNetworkSettingsController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self disableToolbar];
    self.hostDenylist = DSPNetworkRecorder.defaultRecorder.hostDenylist.mutableCopy;
    ...
    self.observerSwitch.on = DSPNetworkObserver.enabled;
    [self.observerSwitch addTarget:self
        action:@selector(networkDebuggingToggled:)
        forControlEvents:UIControlEventValueChanged
    ];
    
    self.cacheMediaSwitch.on = DSPNetworkRecorder.defaultRecorder.shouldCacheMediaResponses;
    [self.cacheMediaSwitch addTarget:self
        action:@selector(cacheMediaResponsesToggled:)
        forControlEvents:UIControlEventValueChanged
    ];
    
    self.jsonViewerSwitch.on = defaults.dsp_registerDictionaryJSONViewerOnLaunch;
    ...
    self.cacheLimitValue = DSPNetworkRecorder.defaultRecorder.responseCacheByteLimit;
...
- (void)setCacheLimitValue:(float)cacheLimitValue {
    ...
    [DSPNetworkRecorder.defaultRecorder setResponseCacheByteLimit:cacheLimitValue];
}
...
- (void)networkDebuggingToggled:(UISwitch *)sender {
    DSPNetworkObserver.enabled = sender.isOn;
}

- (void)cacheMediaResponsesToggled:(UISwitch *)sender {
    DSPNetworkRecorder.defaultRecorder.shouldCacheMediaResponses = sender.isOn;
}

- (void)jsonViewerSettingToggled:(UISwitch *)sender {
    [NSUserDefaults.standardUserDefaults dsp_toggleBoolForKey:kDSPDefaultsRegisterJSONExplorerKey];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath  {
    UITableViewCell *cell = [self.tableView
        dequeueReusableCellWithIdentifier:kDSPDefaultCell forIndexPath:indexPath
    ];
        ...
        case 0: {
            switch (indexPath.row) {
                ...
                case 4:
                    ...
                    CGFloat sliderOriginY = DSPFloor((container.size.height - slider.frame.size.height) / 2.0);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [DSPAlert makeAlert:^(DSPAlert *make) {
        ...
        make.button(@"Reset").destructiveStyle().handler(^(NSArray<NSString *> *strings) {
            ...
            [DSPNetworkRecorder.defaultRecorder.hostDenylist removeAllObjects];
...
forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [DSPNetworkRecorder.defaultRecorder.hostDenylist removeObject:host];
    [DSPNetworkRecorder.defaultRecorder synchronizeDenylist];
