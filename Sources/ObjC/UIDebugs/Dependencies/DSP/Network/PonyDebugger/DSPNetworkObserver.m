...
NSString *const kDSPNetworkObserverEnabledStateChangedNotification = @"kDSPNetworkObserverEnabledStateChangedNotification";
...
@interface DSPInternalRequestState : NSObject
...
@implementation DSPInternalRequestState

@end

@interface DSPNetworkObserver (NSURLConnectionHelpers)
...
@interface DSPNetworkObserver (NSURLSessionTaskHelpers)
...
- (void)websocketTask:(NSURLSessionWebSocketTask *)task
      sendMessage:(NSURLSessionWebSocketMessage *)message API_AVAILABLE(ios(13.0));
- (void)websocketTaskMessageSendCompletion:(NSURLSessionWebSocketMessage *)message
                                     error:(NSError *)error API_AVAILABLE(ios(13.0));

- (void)websocketTask:(NSURLSessionWebSocketTask *)task
       receiveMessage:(NSURLSessionWebSocketMessage *)message
        error:(NSError *)error API_AVAILABLE(ios(13.0));

@end

@interface DSPNetworkObserver ()

@property (nonatomic) NSMutableDictionary<NSString *, DSPInternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic) dispatch_queue_t queue;

@end

@implementation DSPNetworkObserver
...
+ (void)setEnabled:(BOOL)enabled {
    ...
    NSUserDefaults.standardUserDefaults.dsp_networkObserverEnabled = enabled;
    ...
    if (previouslyEnabled != enabled) {
        [NSNotificationCenter.defaultCenter postNotificationName:kDSPNetworkObserverEnabledStateChangedNotification object:self];
    }
}

+ (BOOL)isEnabled {
    return NSUserDefaults.standardUserDefaults.dsp_networkObserverEnabled;
}
...
+ (instancetype)sharedObserver {
    static DSPNetworkObserver *sharedObserver = nil;
...
+ (void)sniffWithoutDuplicationForObject:(NSObject *)object selector:(SEL)selector
    ...
    // See https://github.com/Flipboard/DSP/issues/61 for an example.
    if (!object) {
...
static void _logos_method$_ungrouped$FIRDocumentReference$getDocumentWithCompletion$(
    ...
    NSString *requestID = [DSPNetworkObserver nextRequestID];
    
    // Record transaction start
    [DSPNetworkRecorder.defaultRecorder recordFIRDocumentWillFetch:self withTransactionID:requestID];
    // Hook callback
    FIRDocumentSnapshotBlock orig = completion;
    completion = ^(FIRDocumentSnapshot *document, NSError *error) {
        [DSPNetworkRecorder.defaultRecorder recordFIRDocumentDidFetch:document error:error transactionID:requestID];
        if (orig != nil) {
            orig(document, error);
        }
    };
...
static void _logos_method$_ungrouped$FIRQuery$getDocumentsWithCompletion$(
    ...
    // Record transaction start
    [DSPNetworkRecorder.defaultRecorder recordFIRQueryWillFetch:self withTransactionID:requestID];
    // Hook callback
    FIRQuerySnapshotBlock orig = completion;
    completion = ^(FIRQuerySnapshot *query, NSError *error) {
        [DSPNetworkRecorder.defaultRecorder recordFIRQueryDidFetch:query error:error transactionID:requestID];
        if (orig != nil) {
            orig(query, error);
        }
    };
...
static void _logos_method$_ungrouped$FIRDocumentReference$setData$merge$completion$(
    ...
    // Record transaction start
    [DSPNetworkRecorder.defaultRecorder
    ...
    completion = ^(NSError *error) {
        [DSPNetworkRecorder.defaultRecorder recordFIRDidSetData:error transactionID:requestID];
        if (orig != nil) {
            orig(error);
        }
    };
...
static void _logos_method$_ungrouped$FIRDocumentReference$updateData$completion$(
    ...
    // Record transaction start
    [DSPNetworkRecorder.defaultRecorder recordFIRWillUpdateData:self fields:fields transactionID:requestID];
    // Hook callback
    void (^orig)(NSError *) = completion;
    completion = ^(NSError *error) {
        [DSPNetworkRecorder.defaultRecorder recordFIRDidUpdateData:error transactionID:requestID];
        if (orig != nil) {
            orig(error);
        }
    };
...
static void _logos_method$_ungrouped$FIRDocumentReference$deleteDocumentWithCompletion$(
    ...
    // Record transaction start
    [DSPNetworkRecorder.defaultRecorder recordFIRWillDeleteDocument:self transactionID:requestID];
    // Hook callback
    void (^orig)(NSError *) = completion;
    completion = ^(NSError *error) {
        [DSPNetworkRecorder.defaultRecorder recordFIRDidDeleteDocument:error transactionID:requestID];
        if (orig != nil) {
            orig(error);
        }
    };
...
static FIRDocumentReference * _logos_method$_ungrouped$FIRCollectionReference$addDocumentWithData$completion$(
    ...
    void (^orig)(NSError *) = completion;
    completion = ^(NSError *error) {
        [DSPNetworkRecorder.defaultRecorder recordFIRDidAddDocument:error transactionID:requestID];
        if (orig != nil) {
            orig(error);
        }
    };
    ...
    // Record transaction start
    [DSPNetworkRecorder.defaultRecorder recordFIRWillAddDocument:self document:ret transactionID:requestID];
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [DSPNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = [DSPUtility swizzledSelectorForSelector:selector];
        Method originalCancel = class_getInstanceMethod(class, selector);

        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [DSPNetworkObserver.sharedObserver connectionWillCancel:slf];
...
+ (void)injectIntoNSURLSessionTaskResume {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (sel_af_resume) {
            NSMutableArray<Class> *classTree = DSPGetAllSubclasses(baseResumeClass, NO).mutableCopy;
            for (NSInteger i = 0; i < classTree.count; i++) {
                [classTree addObjectsFromArray:DSPGetAllSubclasses(classTree[i], NO)];
            }
...
+ (void)swizzleResumeSelector:(SEL)selector forClass:(Class)class {
    ...
    IMP implementation = imp_implementationWithBlock(^(NSURLSessionTask *slf) {
        ...
        // -originalRequest will crash. Do not try to observe these. https://github.com/DSPool/DSP/issues/276
        if (![slf isKindOfClass:[AVAggregateAssetDownloadTask class]]) {
            ...
            [DSPNetworkObserver.sharedObserver URLSessionTaskWillResume:slf];
        }
    ...
    dispatch_once(&onceToken, ^{
        ...
        SendAsyncRequestBlock swizzleBlock = ^(Class slf,
            ...
            if (DSPNetworkObserver.isEnabled) {
                ...
                [DSPNetworkRecorder.defaultRecorder recordMechanism:mechanism forRequestID:requestID];
        ...
        [DSPUtility replaceImplementationOfKnownSelector:selector
...
+ (void)injectIntoNSURLSessionAsyncDataAndDownloadTaskMethods:(Class)sessionClass {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            if ([DSPUtility instanceRespondsButDoesNotImplementSelector:selector class:class]) {
            ...
            NSURLSessionNewTaskMethod swizzleBlock = ^NSURLSessionTask *(NSURLSession *slf,
                ...
                if (DSPNetworkObserver.isEnabled && completion) {
                    NSString *requestID = [self nextRequestID];
                    NSString *mechanism = [self mechanismFromClassMethod:selector onClass:class];
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls {
    ...
    WillSendRequestBlock undefinedBlock = ^NSURLRequest *(id slf,
                                                          ...
                                                          NSURLRequest *request,
                                                          NSURLResponse *response) {
        [DSPNetworkObserver.sharedObserver
    ...
    [DSPUtility replaceImplementationOfSelector:selector
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls {
    ...
    DidReceiveResponseBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf,
                                               NSURLConnection *connection,
                                               NSURLResponse *response) {
        [DSPNetworkObserver.sharedObserver connection:connection
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls {
    ...
    FinishLoadingBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf, NSURLConnection *connection) {
        [DSPNetworkObserver.sharedObserver connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls {
    ...
    DidReceiveDataBlock undefinedBlock = ^(id<NSURLSessionDataDelegate> slf,
                                           ...
                                           NSURLSessionDataTask *dataTask,
                                           NSData *data) {
        [DSPNetworkObserver.sharedObserver URLSession:session
...
+ (void)injectWebsocketSendMessage:(Class)cls API_AVAILABLE(ios(13.0)) {
    ...
    SendMessageBlock implementationBlock = ^(
        ...
        [DSPNetworkObserver.sharedObserver
            websocketTask:slf sendMessage:message
        ];
...
+ (void)injectWebsocketReceiveMessage:(Class)cls API_AVAILABLE(ios(13.0)) {
    ...
    SendMessageBlock implementationBlock = ^(
        ...
        id completionHook = ^(NSURLSessionWebSocketMessage *message, NSError *error) {
            [DSPNetworkObserver.sharedObserver
                websocketTask:slf receiveMessage:message error:error
            ];
...
static char const * const kDSPRequestIDKey = "kDSPRequestIDKey";

+ (NSString *)requestIDForConnectionOrTask:(id)connectionOrTask {
    NSString *requestID = objc_getAssociatedObject(connectionOrTask, kDSPRequestIDKey);
    if (!requestID) {
...
+ (void)setRequestID:(NSString *)requestID forConnectionOrTask:(id)connectionOrTask {
    objc_setAssociatedObject(
        connectionOrTask, kDSPRequestIDKey, requestID, OBJC_ASSOCIATION_RETAIN_NONATOMIC
    );
}
...
- (id)init {
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create(
            "com.dsp.DSPNetworkObserver", DISPATCH_QUEUE_SERIAL
        );
    }
...
- (DSPInternalRequestState *)requestStateForRequestID:(NSString *)requestID {
    DSPInternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [DSPInternalRequestState new];
...
@implementation DSPNetworkObserver (NSURLConnectionHelpers)
...
- (void)connection:(NSURLConnection *)connection
  ...
  redirectResponse:(NSURLResponse *)response
    ...
    [self performBlock:^{
        ...
        DSPInternalRequestState *requestState = [self requestStateForRequestID:requestID];
...
@implementation DSPNetworkObserver (NSURLSessionTaskHelpers)
...
- (void)URLSession:(NSURLSession *)session
    ...
    [self performBlock:^{
        ...
        // https://github.com/DSPool/DSP/issues/568#issuecomment-1141015572
...
- (void)websocketTask:(NSURLSessionWebSocketTask *)task
    ...
    [self performBlock:^{
//        NSString *requestID = [[self class] requestIDForConnectionOrTask:task];
        [DSPNetworkRecorder.defaultRecorder recordWebsocketMessageSend:message task:task];
    }];
}
