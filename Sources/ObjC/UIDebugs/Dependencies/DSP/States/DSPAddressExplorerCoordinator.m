...
@interface UITableViewController (DSPAddressExploration)
- (void)deselectSelectedRow;
- (void)tryExploreAddress:(NSString *)addressString safely:(BOOL)safely;
@end

@implementation DSPAddressExplorerCoordinator
...
@implementation UITableViewController (DSPAddressExploration)
...
- (void)tryExploreAddress:(NSString *)addressString safely:(BOOL)safely {
    ...
    if (didParseAddress) {
        if (safely && ![DSPRuntimeUtility pointerIsValidObjcObject:pointerValue]) {
    ...
    if (!error) {
        ...
        DSPObjectExplorerViewController *explorer = [DSPObjectExplorerFactory explorerViewControllerForObject:object];
        [self.navigationController pushViewController:explorer animated:YES];
    } else {
        [DSPAlert showAlert:@"Uh-oh" message:error from:self];
