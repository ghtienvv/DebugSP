...
static const NSInteger kDSPLiveObjectsSortAlphabeticallyIndex = 0;
static const NSInteger kDSPLiveObjectsSortByCountIndex = 1;
static const NSInteger kDSPLiveObjectsSortBySizeIndex = 2;

@interface DSPLiveObjectsController ()
...
@implementation DSPLiveObjectsController
...
+ (UIViewController *)globalsEntryViewController:(DSPGlobalsRow)row {
    DSPLiveObjectsController *liveObjectsViewController = [self new];
...
- (void)updateSearchResults:(NSString *)filter {
    ...
    if (selectedScope == kDSPLiveObjectsSortAlphabeticallyIndex) {
        self.filteredClassNames = [self.filteredClassNames sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)];
    } else if (selectedScope == kDSPLiveObjectsSortByCountIndex) {
    ...
    } else if (selectedScope == kDSPLiveObjectsSortBySizeIndex) {
        self.filteredClassNames = [self.filteredClassNames sortedArrayUsingComparator:^NSComparisonResult(NSString *className1, NSString *className2) {
            NSNumber *count1 = self.instanceCountsForClassNames[className1];
            NSNumber *count2 = self.instanceCountsForClassNames[className2];
...
- (UITableViewCell *)tableView:(__kindof UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView
        dequeueReusableCellWithIdentifier:kDSPDefaultCell
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    NSString *className = self.filteredClassNames[indexPath.row];
    UIViewController *instances = [DSPObjectListViewController
