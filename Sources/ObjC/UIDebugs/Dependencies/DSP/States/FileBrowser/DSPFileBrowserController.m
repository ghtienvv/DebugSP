...
@interface DSPFileBrowserTableViewCell : UITableViewCell
@end

typedef NS_ENUM(NSUInteger, DSPFileBrowserSortAttribute) {
    DSPFileBrowserSortAttributeNone = 0,
    DSPFileBrowserSortAttributeName,
    DSPFileBrowserSortAttributeCreationDate,
};

@interface DSPFileBrowserController () <DSPFileBrowserSearchOperationDelegate>
...
@property (nonatomic) DSPFileBrowserSortAttribute sortAttribute;

@end

@implementation DSPFileBrowserController
...
- (void)viewDidLoad {
    ...
    self.searchBarDebounceInterval = kDSPDebounceForAsyncSearch;
...
- (void)sortDidTouchUpInside:(UIBarButtonItem *)sortButton {
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"None"
        ...
        [self sortWithAttribute:DSPFileBrowserSortAttributeNone];
    }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"Name"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction * _Nonnull action) {
        [self sortWithAttribute:DSPFileBrowserSortAttributeName];
    }]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"Creation Date"
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction * _Nonnull action) {
        [self sortWithAttribute:DSPFileBrowserSortAttributeCreationDate];
    }]];
    [self presentViewController:alertController animated:YES completion:nil];
}

- (void)sortWithAttribute:(DSPFileBrowserSortAttribute)attribute {
...
+ (NSString *)globalsEntryTitle:(DSPGlobalsRow)row {
    switch (row) {
        case DSPGlobalsRowBrowseBundle: return @"📁  Browse Bundle Directory";
        case DSPGlobalsRowBrowseContainer: return @"📁  Browse Container Directory";
        default: return nil;
    }
}
...
+ (UIViewController *)globalsEntryViewController:(DSPGlobalsRow)row {
    switch (row) {
        case DSPGlobalsRowBrowseBundle: return [[self alloc] initWithPath:NSBundle.mainBundle.bundlePath];
        case DSPGlobalsRowBrowseContainer: return [[self alloc] initWithPath:NSHomeDirectory()];
        default: return [self new];
    }
}

#pragma mark - DSPFileBrowserSearchOperationDelegate
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString * const kDSPFileBrowserTextCellIdentifier = @"textCell";
    static NSString * const kDSPFileBrowserImageCellIdentifier = @"imageCell";
    ...
    NSString *cellIdentifier = image ? kDSPFileBrowserImageCellIdentifier : kDSPFileBrowserTextCellIdentifier;

    if (!cell) {
        cell = [[DSPFileBrowserTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellIdentifier];
        cell.textLabel.font = UIFont.dsp_defaultTableCellFont;
        cell.detailTextLabel.font = UIFont.dsp_defaultTableCellFont;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!stillExists) {
        [DSPAlert showAlert:@"File Not Found" message:@"The file at the specified path no longer exists." from:self];
        [self reloadDisplayedPaths];
        return;
    }
    ...
    if (isDirectory) {
        ...
        drillInViewController = [DSPImagePreviewViewController forImage:image];
    } else {
        NSData *fileData = [NSData dataWithContentsOfFile:fullPath];
        if (!fileData.length) {
            [DSPAlert showAlert:@"Empty File" message:@"No data returned from the file." from:self];
            return;
        }
        ...
        if ([pathExtension isEqualToString:@"json"]) {
            prettyString = [DSPUtility prettyJSONStringFromData:fileData];
        } else {
            ...
            if (object) {
                drillInViewController = [DSPObjectExplorerFactory explorerViewControllerForObject:object];
            } else {
                ...
                if (fileData.length > sizeof(struct mach_header_64)) {
                    ...
                    if (header.magic == MH_MAGIC_64) {
                        ...
                        if (count > 0) {
                            NSArray<NSString *> *classNames = [NSArray dsp_forEachUpTo:count map:^id(NSUInteger i) {
                                return objc_getClass(classList[i]);
                            }];
                            drillInViewController = [DSPObjectExplorerFactory explorerViewControllerForObject:classNames];
                        }
                    }
                }
            }
        ...
        if (prettyString.length) {
            drillInViewController = [[DSPWebViewController alloc] initWithText:prettyString];
        } else if ([DSPWebViewController supportsPathExtension:pathExtension]) {
            drillInViewController = [[DSPWebViewController alloc] initWithURL:[NSURL fileURLWithPath:fullPath]];
        } else if ([DSPTableListViewController supportsExtension:pathExtension]) {
            drillInViewController = [[DSPTableListViewController alloc] initWithPath:fullPath];
        }
        else if (!drillInViewController) {
            NSString *fileString = [NSString stringWithUTF8String:fileData.bytes];
            if (fileString.length) {
                drillInViewController = [[DSPWebViewController alloc] initWithText:fileString];
            }
        }
    }
...
- (void)fileBrowserRename:(UITableViewCell *)sender {
    ...
    if (stillExists) {
        ...
        [DSPAlert showAlert:@"File Removed" message:@"The file at the specified path no longer exists." from:self];
    }
}
...
- (void)fileBrowserShare:(UITableViewCell *)sender {
    ...
    if (isDirectory) {
        ...
        UIViewController *shareSheet = [DSPActivityViewController sharing:@[filePath] source:sender];
...
- (void)reloadCurrentPath {
    ...
    if (self.sortAttribute != DSPFileBrowserSortAttributeNone) {
        [childPaths sortUsingComparator:^NSComparisonResult(NSString *path1, NSString *path2) {
            switch (self.sortAttribute) {
                case DSPFileBrowserSortAttributeNone:
                    // invalid state
                    return NSOrderedSame;
                case DSPFileBrowserSortAttributeName:
                    return [path1 compare:path2];
                case DSPFileBrowserSortAttributeCreationDate: {
...
- (void)updateSearchPaths {
    ...
    [self.operationQueue cancelAllOperations];
    DSPFileBrowserSearchOperation *newOperation = [[DSPFileBrowserSearchOperation alloc] initWithPath:self.path searchString:self.searchText];
...
@implementation DSPFileBrowserTableViewCell
