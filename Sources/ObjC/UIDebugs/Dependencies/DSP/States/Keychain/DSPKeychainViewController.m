...
@interface DSPKeychainViewController ()
@property (nonatomic, readonly) DSPMutableListSection<NSDictionary *> *section;
@end

@implementation DSPKeychainViewController
...
- (void)viewDidLoad {
    ...
    [self addToolbarItems:@[
        DSPBarButtonItemSystem(Add, self, @selector(addPressed)),
        [DSPBarButtonItemSystem(Trash, self, @selector(trashPressed:)) dsp_withTintColor:UIColor.redColor],
    ]];
...
- (NSArray<DSPTableViewSection *> *)makeSections {
    _section = [DSPMutableListSection list:DSPKeychain.allAccounts.mutableCopy
        cellConfiguration:^(__kindof DSPTableViewCell *cell, NSDictionary *item, NSInteger row) {
            cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
        
            id service = item[kDSPKeychainWhereKey];
            if ([service isKindOfClass:[NSString class]]) {
                cell.textLabel.text = service;
                cell.detailTextLabel.text = [item[kDSPKeychainAccountKey] description];
            } else {
...
- (void)reloadSections {
    self.section.list = DSPKeychain.allAccounts.mutableCopy;
}

- (void)refreshSectionTitle {
    self.section.customTitle = DSPPluralString(
...
- (DSPKeychainQuery *)queryForItemAtIndex:(NSInteger)idx {
    NSDictionary *item = self.section.filteredList[idx];

    DSPKeychainQuery *query = [DSPKeychainQuery new];
    query.service = [item[kDSPKeychainWhereKey] description];
    query.account = [item[kDSPKeychainAccountKey] description];
    query.accessGroup = [item[kDSPKeychainGroupKey] description];
    [query fetch:nil];
...
- (void)deleteItem:(NSDictionary *)item {
    NSError *error = nil;
    BOOL success = [DSPKeychain
        deletePasswordForService:item[kDSPKeychainWhereKey]
        account:item[kDSPKeychainAccountKey]
        error:&error
    ];
...
- (void)addPressed {
    [DSPAlert makeAlert:^(DSPAlert *make) {
        ...
        make.button(@"Save").handler(^(NSArray<NSString *> *strings) {
            // Display errors
            NSError *error = nil;
            if (![DSPKeychain setPassword:strings[2] forService:strings[0] account:strings[1] error:&error]) {
                [DSPAlert showAlert:@"Error" message:error.localizedDescription from:self];
            }
...
+ (UIViewController *)globalsEntryViewController:(DSPGlobalsRow)row {
    DSPKeychainViewController *viewController = [self new];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    DSPKeychainQuery *query = [self queryForItemAtIndex:indexPath.row];
    ...
    [DSPAlert makeAlert:^(DSPAlert *make) {
        ...
        make.button(@"Copy Service").handler(^(NSArray<NSString *> *strings) {
            [UIPasteboard.generalPasteboard dsp_copy:query.service];
        });
        make.button(@"Copy Account").handler(^(NSArray<NSString *> *strings) {
            [UIPasteboard.generalPasteboard dsp_copy:query.account];
        });
        make.button(@"Copy Password").handler(^(NSArray<NSString *> *strings) {
            [UIPasteboard.generalPasteboard dsp_copy:query.password];
        });
