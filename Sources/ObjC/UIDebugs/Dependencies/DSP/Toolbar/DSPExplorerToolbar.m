...
@interface DSPExplorerToolbar ()

@property (nonatomic, readwrite) DSPExplorerToolbarItem *globalsItem;
@property (nonatomic, readwrite) DSPExplorerToolbarItem *hierarchyItem;
@property (nonatomic, readwrite) DSPExplorerToolbarItem *selectItem;
@property (nonatomic, readwrite) DSPExplorerToolbarItem *recentItem;
@property (nonatomic, readwrite) DSPExplorerToolbarItem *moveItem;
@property (nonatomic, readwrite) DSPExplorerToolbarItem *closeItem;
...
@implementation DSPExplorerToolbar
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.backgroundView.backgroundColor = [DSPColor secondaryBackgroundColorWithAlpha:0.95];
        [self addSubview:self.backgroundView];

        // Drag handle
        self.dragHandle = [UIView new];
        self.dragHandle.backgroundColor = UIColor.clearColor;
        self.dragHandleImageView = [[UIImageView alloc] initWithImage:DSPResources.dragHandle];
        self.dragHandleImageView.tintColor = [DSPColor.iconColor colorWithAlphaComponent:0.666];
        [self.dragHandle addSubview:self.dragHandleImageView];
        [self addSubview:self.dragHandle];
        
        // Buttons
        self.globalsItem   = [DSPExplorerToolbarItem itemWithTitle:@"menu" image:DSPResources.globalsIcon];
        self.hierarchyItem = [DSPExplorerToolbarItem itemWithTitle:@"views" image:DSPResources.hierarchyIcon];
        self.selectItem    = [DSPExplorerToolbarItem itemWithTitle:@"select" image:DSPResources.selectIcon];
        self.recentItem    = [DSPExplorerToolbarItem itemWithTitle:@"recent" image:DSPResources.recentIcon];
        self.moveItem      = [DSPExplorerToolbarItem itemWithTitle:@"move" image:DSPResources.moveIcon sibling:self.recentItem];
        self.closeItem     = [DSPExplorerToolbarItem itemWithTitle:@"close" image:DSPResources.closeIcon];

        // Selected view box //
        
        self.selectedViewDescriptionContainer = [UIView new];
        self.selectedViewDescriptionContainer.backgroundColor = [DSPColor tertiaryBackgroundColorWithAlpha:0.95];
...
- (void)layoutSubviews {
    ...
    dragHandleImageFrame.origin.x = DSPFloor((self.dragHandle.frame.size.width - dragHandleImageFrame.size.width) / 2.0);
    dragHandleImageFrame.origin.y = DSPFloor((self.dragHandle.frame.size.height - dragHandleImageFrame.size.height) / 2.0);
    ...
    CGFloat width = DSPFloor((CGRectGetWidth(safeArea) - CGRectGetWidth(self.dragHandle.frame)) / self.toolbarItems.count);
    for (DSPExplorerToolbarItem *toolbarItem in self.toolbarItems) {
    ...
    selectedViewColorFrame.origin.y = DSPFloor((kDescriptionContainerHeight - kSelectedViewColorDiameter) / 2.0);
...
- (void)setToolbarItems:(NSArray<DSPExplorerToolbarItem *> *)toolbarItems {
    if (_toolbarItems == toolbarItems) {
        return;
    }
    
    // Remove old toolbar items, if any
    for (DSPExplorerToolbarItem *item in _toolbarItems) {
    ...
    for (DSPExplorerToolbarItem *item in toolbarItems) {
...
+ (CGFloat)dragHandleWidth {
    return DSPResources.dragHandle.size.width;
}
