...
BOOL DSPConstructorsShouldRun() {
    #if DSP_DISABLE_CTORS
        return NO;
    #else
        static BOOL _DSPConstructorsShouldRun_storage = YES;
        
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            NSString *key = @"DSP_SKIP_INIT";
            if (getenv(key.UTF8String) || [NSUserDefaults.standardUserDefaults boolForKey:key]) {
                _DSPConstructorsShouldRun_storage = NO;
            }
        });
        
        return _DSPConstructorsShouldRun_storage;
    #endif
}

@implementation DSPUtility

+ (UIWindow *)appKeyWindow {
    // First, check UIApplication.keyWindow
    DSPWindow *window = (id)UIApplication.sharedApplication.keyWindow;
    if (window) {
    ...
    //
    // Also, these are obviously not all DSPWindows; DSPWindow is used
    // so we can call window.previousKeyWindow without an ugly cast
    for (DSPWindow *window in UIApplication.sharedApplication.windows) {
...
+ (UIColor *)hierarchyIndentPatternColor {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        UIImage *indentationPatternImage = DSPResources.hierarchyIndentPattern;
...
+ (NSString *)applicationName {
    return DSPUtility.applicationImageName.lastPathComponent;
}
...
+ (SEL)swizzledSelectorForSelector:(SEL)selector {
    return NSSelectorFromString([NSString stringWithFormat:
        @"_dsp_swizzle_%x_%@", arc4random(), NSStringFromSelector(selector)
    ]);
}
