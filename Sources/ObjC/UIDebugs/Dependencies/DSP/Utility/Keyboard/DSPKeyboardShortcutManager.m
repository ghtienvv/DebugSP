...
@interface DSPKeyInput : NSObject <NSCopying>
...
@implementation DSPKeyInput

- (BOOL)isEqual:(id)object {
    BOOL isEqual = NO;
    if ([object isKindOfClass:[DSPKeyInput class]]) {
        DSPKeyInput *keyCommand = (DSPKeyInput *)object;
...
@interface DSPKeyboardShortcutManager ()

@property (nonatomic) NSMutableDictionary<DSPKeyInput *, dispatch_block_t> *actionsForKeyInputs;
...
@implementation DSPKeyboardShortcutManager

+ (instancetype)sharedManager {
    static DSPKeyboardShortcutManager *sharedManager = nil;
...
+ (void)load {
    ...
    SEL swizzledKeyEventSelector = [DSPUtility swizzledSelectorForSelector:originalKeyEventSelector];
    ...
    [DSPUtility replaceImplementationOfKnownSelector:originalKeyEventSelector
    ...
    if ([[UITouch class] instancesRespondToSelector:@selector(maximumPossibleForce)]) {
        ...
        SEL swizzledSendEventSelector = [DSPUtility swizzledSelectorForSelector:originalSendEventSelector];
        
        void (^sendEventSwizzleBlock)(UIApplication *, UIEvent *) = ^(UIApplication *slf, UIEvent *event) {
            if (event.type == UIEventTypeTouches) {
                DSPKeyboardShortcutManager *keyboardManager = DSPKeyboardShortcutManager.sharedManager;
        ...
        [DSPUtility replaceImplementationOfKnownSelector:originalSendEventSelector
            onClass:[UIApplication class]
        ...
        SEL swizzledSupportsTouchPressureSelector = [DSPUtility swizzledSelectorForSelector:originalSupportsTouchPressureSelector];
        
        BOOL (^supportsTouchPressureSwizzleBlock)(UIDevice *) = ^BOOL(UIDevice *slf) {
            return YES;
        };
        
        [DSPUtility replaceImplementationOfKnownSelector:originalSupportsTouchPressureSelector
...
- (void)registerSimulatorShortcutWithKey:(NSString *)key
    ...
    DSPKeyInput *keyInput = [DSPKeyInput keyInputForKey:key flags:modifiers helpDescription:description];
...
static const long kDSPControlKeyCode = 0xe0;
static const long kDSPShiftKeyCode = 0xe1;
static const long kDSPCommandKeyCode = 0xe3;
...
- (void)handleKeyboardEvent:(UIEvent *)event {
    ...
    if (isKeyDown && modifiedInput.length > 0 && interactionEnabled) {
        ...
        if (firstResponder) {
            ...
            DSPKeyInput *exactMatch = [DSPKeyInput keyInputForKey:unmodifiedInput flags:flags];
            
            dispatch_block_t actionBlock = self.actionsForKeyInputs[exactMatch];
            
            if (!actionBlock) {
                DSPKeyInput *shiftMatch = [DSPKeyInput
            ...
            if (!actionBlock) {
                DSPKeyInput *capitalMatch = [DSPKeyInput
    ...
    if (!hasFirstResponder && [event respondsToSelector:@selector(_keyCode)]) {
        ...
        if (keyCode == kDSPControlKeyCode) {
            self.pressingControl = isKeyDown;
        } else if (keyCode == kDSPCommandKeyCode) {
            self.pressingCommand = isKeyDown;
        } else if (keyCode == kDSPShiftKeyCode) {
...
- (NSString *)keyboardShortcutsDescription {
    ...
    NSArray<DSPKeyInput *> *keyInputs = [self.actionsForKeyInputs.allKeys
        sortedArrayUsingComparator:^NSComparisonResult(DSPKeyInput *input1, DSPKeyInput *input2) {
            return [input1.key caseInsensitiveCompare:input2.key];
        }
    ];
    for (DSPKeyInput *keyInput in keyInputs) {
