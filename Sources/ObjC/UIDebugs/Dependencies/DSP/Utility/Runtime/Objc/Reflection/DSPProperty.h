...
@class DSPPropertyAttributes, DSPMethodBase;


#pragma mark DSPProperty
@interface DSPProperty : NSObject
...
+ (instancetype)propertyWithName:(NSString *)name attributes:(DSPPropertyAttributes *)attributes;
...
@property (nonatomic, readonly) DSPTypeEncoding type;
/// The property's attributes.
@property (nonatomic          ) DSPPropertyAttributes *attributes;
...
/// -[DSPRuntimeUtility potentiallyUnwrapBoxedPointer:type:]
/// and returns the result.
///
/// If this is a class property, you must class the class object.
- (id)getPotentiallyUnboxedValue:(id)target;

/// Safe to use regardless of how the \c DSPProperty instance was initialized.
///
...
/// @discussion Consider using the \c DSPPropertyGetter macros.
- (DSPMethodBase *)getterWithImplementation:(IMP)implementation;
/// @return A setter for the property with the given implementation.
/// @discussion Consider using the \c DSPPropertySetter macros.
- (DSPMethodBase *)setterWithImplementation:(IMP)implementation;

#pragma mark DSPMethod property getter / setter macros
// Easier than using the above methods yourself in most cases

/// Takes a \c DSPProperty and a type (ie \c NSUInteger or \c id) and
/// uses the \c DSPProperty's \c attribute's \c backingIvarName to get the Ivar.
#define DSPPropertyGetter(DSPProperty, type) [DSPProperty \
    getterWithImplementation:imp_implementationWithBlock(^(id self) { \
        return *(type *)[self getIvarAddressByName:DSPProperty.attributes.backingIvar]; \
    }) \
];
/// Takes a \c DSPProperty and a type (ie \c NSUInteger or \c id) and
/// uses the \c DSPProperty's \c attribute's \c backingIvarName to set the Ivar.
#define DSPPropertySetter(DSPProperty, type) [DSPProperty \
    setterWithImplementation:imp_implementationWithBlock(^(id self, type value) { \
        [self setIvarByName:DSPProperty.attributes.backingIvar value:&value size:sizeof(type)]; \
    }) \
];
/// Takes a \c DSPProperty and a type (ie \c NSUInteger or \c id) and an Ivar name string to get the Ivar.
#define DSPPropertyGetterWithIvar(DSPProperty, ivarName, type) [DSPProperty \
    getterWithImplementation:imp_implementationWithBlock(^(id self) { \
        return *(type *)[self getIvarAddressByName:ivarName]; \
    }) \
];
/// Takes a \c DSPProperty and a type (ie \c NSUInteger or \c id) and an Ivar name string to set the Ivar.
#define DSPPropertySetterWithIvar(DSPProperty, ivarName, type) [DSPProperty \
    setterWithImplementation:imp_implementationWithBlock(^(id self, type value) { \
