...
- (void)examine {
    ...
    _protocols = [NSArray dsp_forEachUpTo:pccount map:^id(NSUInteger i) {
        return [DSPProtocol protocol:protocols[i]];
    }];
    free(protocols);
    
    // Required instance methods
    objcrMethods = protocol_copyMethodDescriptionList(protocol, YES, YES, &mdrcount);
    NSArray *rMethods = [NSArray dsp_forEachUpTo:mdrcount map:^id(NSUInteger i) {
        return [DSPMethodDescription description:objcrMethods[i] instance:YES];
    }];
    free(objcrMethods);
    
    // Required class methods 
    objcrMethods = protocol_copyMethodDescriptionList(protocol, YES, NO, &mdrcount);
    _requiredMethods = [[NSArray dsp_forEachUpTo:mdrcount map:^id(NSUInteger i) {
        return [DSPMethodDescription description:objcrMethods[i] instance:NO];
    }] arrayByAddingObjectsFromArray:rMethods];
    free(objcrMethods);
    
    // Optional instance methods
    objcoMethods = protocol_copyMethodDescriptionList(protocol, NO, YES, &mdocount);
    NSArray *oMethods = [NSArray dsp_forEachUpTo:mdocount map:^id(NSUInteger i) {
        return [DSPMethodDescription description:objcoMethods[i] instance:YES];
    }];
    free(objcoMethods);
    
    // Optional class methods
    objcoMethods = protocol_copyMethodDescriptionList(protocol, NO, NO, &mdocount);
    _optionalMethods = [[NSArray dsp_forEachUpTo:mdocount map:^id(NSUInteger i) {
        return [DSPMethodDescription description:objcoMethods[i] instance:NO];
    ...
    if (@available(iOS 10.0, *)) {
        ...
        NSArray *rProperties = [NSArray dsp_forEachUpTo:prrcount map:^id(NSUInteger i) {
            return [DSPProperty property:rProps[i] onClass:instance];
        }];
        free(rProps);
        
        // Then class
        rProps = protocol_copyPropertyList2(protocol, &prrcount, NO, YES);
        _requiredProperties = [[NSArray dsp_forEachUpTo:prrcount map:^id(NSUInteger i) {
            return [DSPProperty property:rProps[i] onClass:instance];
        ...
        NSArray *oProperties = [NSArray dsp_forEachUpTo:prrcount map:^id(NSUInteger i) {
            return [DSPProperty property:oProps[i] onClass:meta];
        }];
        free(oProps);
        
        // Then class
        oProps = protocol_copyPropertyList2(protocol, &procount, NO, YES);
        _optionalProperties = [[NSArray dsp_forEachUpTo:procount map:^id(NSUInteger i) {
            return [DSPProperty property:oProps[i] onClass:meta];
        ...
        _properties = [NSArray dsp_forEachUpTo:prcount map:^id(NSUInteger i) {
            return [DSPProperty property:objcproperties[i]];
        }];
...
#pragma mark DSPMethodDescription

@implementation DSPMethodDescription
...
- (id)initWithDescription:(struct objc_method_description)md instance:(NSNumber *)instance {
    ...
    if (self) {
        ...
        _returnType       = (DSPTypeEncoding)[self.typeEncoding characterAtIndex:0];
        _instance         = instance;
    }
