...
BOOL const kFHSViewControllerExcludeDSPWindows = YES;
...
- (id)initWithViews:(NSArray<UIView *> *)views
    ...
    if (self) {
        ...
        if (!viewsAtTap && kFHSViewControllerExcludeDSPWindows) {
            Class dspwindow = [DSPWindow class];
            views = [views dsp_filtered:^BOOL(UIView *view, NSUInteger idx) {
                return [view class] != dspwindow;
            }];
        }

        _targetViews = views;
        _views = [views dsp_mapped:^id(UIView *view, NSUInteger idx) {
...
- (void)refreshSnapshotView {
    ...
    UIAlertController *loading = [DSPAlert makeAlert:^(DSPAlert *make) {
        make.title(@"Please Wait").message(@"Generating snapshot…");
    }];
    [self presentViewController:loading animated:YES completion:^{
        self.snapshots = [self.views dsp_mapped:^id(FHSView *view, NSUInteger idx) {
...
- (void)loadView {
    [super loadView];
    self.view.backgroundColor = DSPColor.primaryBackgroundColor;
}
...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem
        dsp_itemWithImage:DSPResources.toggle2DIcon
...
- (void)setSnapshotView:(FHSSnapshotView *)snapshotView {
    ...
    self.toolbarItems = @[
        [UIBarButtonItem dsp_itemWithCustomView:snapshotView.spacingSlider],
        UIBarButtonItem.dsp_dspibleSpace,
        [UIBarButtonItem
            dsp_itemWithImage:DSPResources.moreIcon
            target:self action:@selector(didPressOptionsButton:)
        ],
        UIBarButtonItem.dsp_dspibleSpace,
        [UIBarButtonItem dsp_itemWithCustomView:snapshotView.depthSlider]
    ];
