...
public class DSPFloatingItemGestureRecognizer: UIPanGestureRecognizer, UIGestureRecognizerDelegate {
    private weak var groundView: UIView?
    private var gestureGap: CGPoint?
    private let margin: CGFloat = 16
    public var shouldReceiveTouch: ((UITouch, UIView) -> Bool)?
    public enum Edge {
        case top
        case bottom
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch)
        -> Bool
    {
        guard let targetView = self.view else { return true }
        return shouldReceiveTouch?(touch, targetView) ?? true
    }
