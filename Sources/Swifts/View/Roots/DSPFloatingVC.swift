...
internal class DSPFloatingVC: UIViewController {
    ...
    private let memoryWidgetView: DSPMemoryWidgetView
    ...
    override func loadView() {
        ...
        view.addSubview(memoryWidgetView)
        
        launchView.isHidden = true
        memoryWidgetView.isHidden = true
    }
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        memoryWidgetView.onRequestClose = { [weak self] in
        ...
        widget: do {
            ...
            gesture.shouldReceiveTouch = { touch, targetView in
                guard let widgetView = targetView as? DSPMemoryWidgetView else { return true }
                return widgetView.shouldBeginDragging(with: touch)
            }
            memoryWidgetView.addGestureRecognizer(gesture)
    ...
    func updateConfiguration(
        ...
        memoryWidgetView.update(dashboardItems: dashboardItems, configuration: widgetConfiguration)
        applyWidgetVisibility(isInitial: false)
    }
    
    var isWidgetVisible: Bool {
        !memoryWidgetView.isHidden
    }
    
    func setWidgetVisible(_ isVisible: Bool) {
        if isVisible {
            memoryWidgetView.show()
        } else {
            memoryWidgetView.hide()
        }
    }
